// Регистрация Правила регистрации для конфигурации ERP 2.5.26 от 26.12.2025 17:44:03
...
Функция ПараметрыРегистрации() Экспорт
	...
	Наименование = НСтр("ru = 'Правила регистрации для конфигурации ERP 2.5.26'");
	Результат.Вставить("Наименование", Наименование);
	Результат.Вставить("ДатаВремяСоздания", Дата(2025,12,26,17,44,0));
...
Процедура ПРО_Документ_ВозвратТоваровОтКлиента_ПередОбработкой(ПРО, Отказ, Объект, ОбъектМетаданных, Выгрузка)
	ДокументОснование = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Объект.Ссылка, "ДокументРеализации");
	Если ЗначениеЗаполнено(ДокументОснование) И ТипЗнч(ДокументОснование) = Тип("ДокументСсылка.РеализацияТоваровУслуг") Тогда
		...
		Если НЕ СтруктураРеквизитов.Проведен ИЛИ СтруктураРеквизитов.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПереходПраваСобственности Тогда 
			Отказ = Истина;
		КонецЕсли;
	КонецЕсли;
КонецПроцедуры
...
Функция Документ_РеализацияТоваровУслуг_ОтборПоСвойствамОбъекта()
	Возврат "<ОтборПоСвойствамОбъекта>
			...
			|		<СвойствоОбъекта>ХозяйственнаяОперация</СвойствоОбъекта>
			|		<Вид>АлгоритмЗначения</Вид>
			|		<ЗначениеКонстанты>Значение = ПредопределенноеЗначение(""Перечисление.ХозяйственныеОперации.ПереходПраваСобственности"");</ЗначениеКонстанты>
			|		<ТаблицаСвойствОбъекта>
			|			<Свойство>
			|				<Наименование>ХозяйственнаяОперация</Наименование>
			|				<Тип>ПеречислениеСсылка.ХозяйственныеОперации</Тип>
