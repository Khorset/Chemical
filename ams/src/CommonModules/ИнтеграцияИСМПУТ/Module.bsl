...
Процедура ОбработкаЗаполненияДокументаМаркировкаТоваровИСМП(ДокументОбъект, ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка) Экспорт
	...
	Если ЗначениеЗаполнено(ДокументОбъект.ВидПродукции) Тогда
		Если ИнтеграцияИСКлиентСервер.ВидПродукцииИспользуетОтчетыОНанесенииКодовМаркировки(ДокументОбъект.ВидПродукции) Тогда
			ДокументОбъект.ОперацияНанесения = Перечисления.ВидыОперацийИСМП.ОтчетОВерификацииНанесенныхКМ;
		ИначеЕсли ДокументОбъект.ОперацияНанесенияТолькоДляНаборов Тогда
...
Функция ОсобенностиУчетаНоменклатуры() Экспорт
	...
	Массив.Добавить(Перечисления.ОсобенностиУчетаНоменклатуры.ЛегкаяПромышленность2026);
	...
	Массив.Добавить(Перечисления.ОсобенностиУчетаНоменклатуры.ТоварыИзНатуральногоМеха);
...
Процедура ОтразитьИзменениеКоличестваВТабличныхЧастяхТоварыСерииРеализацииТоваровУслуг(ДокументОбъект, ПараметрыОкончанияПроверки)
	...
	Если РозничныеПродажиЛокализация.ВидыПродукцииИССМРЦ().Найти(ПараметрыОкончанияПроверки.ВидПродукцииИС) = Неопределено Тогда
		Возврат;
	КонецЕсли;
...
Процедура ЗаполнитьСерииПоШтрихкодамУпаковокПриВводеНаОсновании(ДокументОбъект, КоличественныеПоля)
	...
	Иначе
		Содержимое = ЭлектронноеВзаимодействиеИСМП.Содержимое(ДокументОбъект.ШтрихкодыУпаковок.Выгрузить(),ДокументОбъект.ВидПродукции);
	КонецЕсли;
...
Процедура ЗаполнитьВозвратВОборотИСМПНаОснованииПоступленияТоваровОтКлиента(ДокументОбъект, ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка)
	...
	|			ТОГДА ЗНАЧЕНИЕ(Перечисление.ВидыОперацийИСМП.ВозвратВОборотПриДистанционномСпособеПродажи)
	|		ИНАЧЕ
	|			ЗНАЧЕНИЕ(Перечисление.ВидыОперацийИСМП.ВозвратВОборотТовараВыведенногоИзОборотаВЦеляхНеСвязанныхСРеализацией)
	|	КОНЕЦ КАК Операция
	|ИЗ
	|	Документ.ПоступлениеТоваровОтКлиента КАК ДокументСсылка
	|ГДЕ
...
Процедура ЗаполнитьОтгрузкуТоваровИСМПНаОснованииОтгрузкиТоваровКлиенту(ДокументОбъект, ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка)
	...
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.Номенклатура КАК СправочникНоменклатура
	|		ПО ОтгрузкаТоваровКлиентуТовары.Номенклатура = СправочникНоменклатура.Ссылка
	|		И СправочникНоменклатура.ОсобенностьУчета В
	|		(ЗНАЧЕНИЕ(Перечисление.ОсобенностиУчетаНоменклатуры.МолочнаяПродукцияБезВЕТИС),
	|			ЗНАЧЕНИЕ(Перечисление.ОсобенностиУчетаНоменклатуры.МолочнаяПродукцияПодконтрольнаяВЕТИС))
	|ГДЕ
...
Процедура ЗаполнитьДанныеАдресовПеремещенияМеждуМОДИСМП(ДокументОбъект, ДатаСведений)
	...
	ПараметрыЗаполненияМОД                = ЭлектронноеВзаимодействиеИСМП.ПараметрыУказанияДанныхПоМОДВЭлектронномДокументеУПД_5_02(ДокументОбъект.ВидПродукции);
	ПараметрыЗаполненияМОДПоВидуПродукции = ПараметрыЗаполненияМОД.ПараметрыУказанияМОДПоВидамПродукции.Получить(ДокументОбъект.ВидПродукции);
	...
	Если ПараметрыЗаполненияМОДПоВидуПродукции <> Неопределено Тогда
		...
		ТребуетсяУказаниеКодаФИАСОтправителя = ПараметрыЗаполненияМОДПоВидуПродукции.ТребуетсяУказаниеКодаФИАСПоМОДОтправителя
			И ПараметрыЗаполненияМОДПоВидуПродукции.ТребуетсяУказаниеКодаФИАСПоМОДИндивидуальныхПредпринимателей
			...
			И ПараметрыЗаполненияМОДПоВидуПродукции.ТребуетсяУказаниеКодаФИАСПоМОДЮридическихЛиц
