...
Функция ДанныеПоОперативномуВидуОповещений(ВидОповещения, ТаблицаИсходныхДанных, РежимПроверки = Ложь)
	...
	Для каждого СтрокаРезультат Из ТаблицаРезультатКомпоновки Цикл
		...
		Если ТекстОповещенияОсновной <> ТекстОповещения Тогда // Повторим оповещение на основном языке конфигурации.
			...
			ДанныеВидаОповещений = КонструкторДанныеВидаОповещений(ТекстОповещенияСПодстановкой, 
				КодОсновногоЯзыка,
...
Функция ДанныеПоНеоперативномуВидуОповещений(РеквизитыВидаОповещения)
	...
	Для каждого СтрокаИтог Из ДеревоРезультат.Строки Цикл
		...
		Если ЗначениеЗаполнено(СтрокаИтог.КонтекстОповещения)
			...
			ДанныеВидаОповещений = КонструкторДанныеВидаОповещений(ТекстОповещения,
				КодЯзыкаПользователя,
				Неопределено,
				СтрокаРезультат.ПолучательОповещенияПослеПроверки,
				КомандаПерехода);
...
//		* КомандаПерехода - Неопределено
//			- см. КонструкторКомандыПерехода
Функция КонструкторДанныеВидаОповещений(ТекстОповещения, КодЯзыкаПользователя, КонтекстОповещения = Неопределено, ПолучательОповещенияПослеПроверки = Неопределено, КомандаПерехода = Неопределено)
	
	ДанныеВидаОповещений = Новый Структура;
	ДанныеВидаОповещений.Вставить("ТекстОповещения", ТекстОповещения);
	ДанныеВидаОповещений.Вставить("КодЯзыкаПользователя", КодЯзыкаПользователя);
