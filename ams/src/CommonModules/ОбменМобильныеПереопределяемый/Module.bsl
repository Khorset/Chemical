...
Процедура ШтрихкодыНоменклатурыВXML(ЗаписьXML, Данные)
	...
	ЗаписьXML.ЗаписатьТекст(Строка(Данные[0].Серия.УникальныйИдентификатор()));
	ЗаписьXML.ЗаписатьКонецЭлемента();
...
Функция ШтрихкодыНоменклатурыИзXML(ЧтениеXML)
	...
	МенеджерЗаписи.Серия          = Справочники.СерииНоменклатуры.ПолучитьСсылку(Новый УникальныйИдентификатор(ОбъектXDTO.Серия));
