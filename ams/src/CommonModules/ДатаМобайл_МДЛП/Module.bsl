...
Функция ЭтоДокументМДЛП(ШаблонВидДокумента) Экспорт
	...
	//МассивВидовДокументовМДЛП.Добавить("РеализацияТоваровУслуг"); DMX-11487
	
	Если ДатаМобайл_ОбщийМодуль.ПроверкаИмениКонфигурации("УправлениеАптечнойСетью") Тогда
		Для каждого МетаданныеДокумента Из Метаданные.Документы Цикл
			ВидДокумента = МетаданныеДокумента.Имя;
			
			Если МетаданныеДокумента.ТабличныеЧасти.Найти("Уас_НомераУпаковок") = Неопределено 
				И МассивВидовДокументовМДЛП.Найти(ВидДокумента) = Неопределено Тогда
				
				МассивВидовДокументовМДЛП.Добавить(ВидДокумента);
			КонецЕсли; 
		КонецЦикла;
	КонецЕсли;
...
Функция ТребуетсяСпецОбработка(Об) Экспорт
	...
	//Список.Добавить("РеализацияТоваровУслуг"); DMX-11487
...
Процедура ОбработатьИзменениеТЧПодбор_Приемка(ДокументТСД, ИсходныйДокумент, ИмяТЧПодбор)
	...
	Для каждого Стр Из ТЗСобранныеДанныеПодбор Цикл
		...
		Если МетаданныеРеквизитыТЧ.Найти("Сумма") <> Неопределено
			...
			Модуль_ОбработкаТабличнойЧастиСервер.ОбработатьСтрокуТЧ(СтрокаТабТовары, ПараметрыЗаполнения, Неопределено);
	...
	Для каждого Стр Из ТЗСобранныеДанныеПодбор Цикл
		...
		Если ЗначениеЗаполнено(Стр.Номенклатура) Тогда
			...
			Иначе
				...
				Если ЗначениеЗаполнено(Стр.СерияСсылка) Тогда
					СтруктураСтроки.Серия = Стр.СерияСсылка;
				Иначе
...
Функция ОбработатьВидДокументаРеализацияТоваровУслуг(ИсходныйДокумент, ДокументТСД, ЭтоНовыйДокумент) Экспорт
	...
	РеквизитыШаблона 	= Справочники.ДатаМобайл_ШаблоныДокументов.ПолучитьПараметрыШаблона(Шаблон, "ТабличнуюЧастьПриЗагрузкеПодбор, ИмяТабличнойЧастиПодбор");
...
Процедура ДобавитьКомментарий(Док, Об, Шаблон)
	...
	Попытка
		...
		КомментарийИзШаблона = "" + СокрЛП(Об.Шаблон.АвтоматическийКомментарий) + " (" + ДатаМобайл_ОбщийМодуль.ПолучитьТекущуюДату() + "). " + Символы.ПС;
