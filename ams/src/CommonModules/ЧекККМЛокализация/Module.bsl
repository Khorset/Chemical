...
Процедура ОбработкаПроверкиЗаполнения(Объект, Отказ, ПроверяемыеРеквизиты) Экспорт
	...
	Если Не(Объект.Статус = Перечисления.СтатусыЧековККМ.КОплате
		...
		ЕстьВедениеУчета = Ложь;
		Для Каждого Элем Из ВидыПродукцииСМРЦ Цикл
			ТекОсобенностьУчета = ИнтеграцияИСУТКлиентСервер.ОсобенностьУчетаПоВидуПродукции(Элем);
			Если ЗначениеЗаполнено(ТекОсобенностьУчета) Тогда
				ОсобенностьУчета.Добавить(ТекОсобенностьУчета);
			Иначе
				Продолжить;
			КонецЕсли;
			Если ИнтеграцияИСМПКлиентСерверПовтИсп.ВестиУчетМаркируемойПродукции(ТекОсобенностьУчета) Тогда
				ЕстьВедениеУчета = Истина;
			КонецЕсли;
		КонецЦикла;
		
		Если ЕстьВедениеУчета И РозничныеПродажиЛокализация.КонтролироватьЦенуПродажиТабачнойПродукции(ДатаДокумента) Тогда
			...
			|	ЧекККМТовары КАК ЧекККМТовары
			|ГДЕ
			|	НЕ ЧекККМТовары.Номенклатура.ВидНоменклатуры.ОсобенностьУчета В(&ОсобенностьУчета)
...
Функция ОбработатьДанныеШтрихкодаПотребительскойУпаковки(Форма, ДанныеШтрихкода, ПараметрыСканирования, ПараметрыЗаполнения = Неопределено) Экспорт
	...
	Если РозничныеПродажиКлиентСерверЛокализация.РозничныеПродажиТабачнойПродукцииТолькоПоМРЦ(Форма) 
		И РозничныеПродажиКлиентСерверЛокализация.ВидыПродукцииСМРЦ().Найти(ДанныеШтрихкода.ВидПродукции) <> Неопределено Тогда
