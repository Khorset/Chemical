	...
	Для Каждого СтрокаОсновныхПараметровРасчета Из ДополненнаяТаблицаПараметровРаспределения Цикл
		...
		Для Каждого СтрокаРасчетов Из ДанныеДляРаспределения Цикл
			...
			Если СтрокаРасчетов.ЭтоОплата Тогда
				...
				Если СтрокаРасчетов.Сумма > 0 Тогда
					...
					Если СтрокаОсновныхПараметровРасчета.ЗачетОплатПоДатеПлатежа Тогда
						ТаблицаОстатковВзаиморасчетов.Сортировать("ПорядокЗачетаПоДатеПлатежа ВОЗР, ДатаВозникновения ВОЗР, ПорядокОперации ВОЗР");
					Иначе
						ТаблицаОстатковВзаиморасчетов.Сортировать("ДатаВозникновения ВОЗР, ПорядокОперации ВОЗР, ПорядокЗачетаПоДатеПлатежа ВОЗР");
					КонецЕсли;
					
					Для Каждого ПодходящаяСтрокаОтгрузки Из ТаблицаОстатковВзаиморасчетов Цикл
						Если СтрокаРасчетов.Сумма = 0 Тогда
							Прервать;
						КонецЕсли;
			...
			Иначе
				...
				Для Каждого ПодходящаяСтрокаОплаты Из ТаблицаОстатковВзаиморасчетов Цикл
					Если СтрокаРасчетов.Сумма = 0 Тогда
						Прервать;
					КонецЕсли;
...
Процедура ВыполнитьОтложенноеРаспределение(Параметры = Неопределено, АдресРезультата = Неопределено) Экспорт
	...
	Для Каждого СтрокаТаблицы Из ДанныеКОбработке Цикл
		...
		Если СтрокаТаблицы.ПорядокОперации = "" Тогда
			СтрокаТаблицы.ПорядокОперации = Порядок(СтрокаТаблицы.ДатаНачалаПересчета, "", Тип("ДокументСсылка.КорректировкаРегистров"), "0", "00");
		КонецЕсли;
...
Процедура ДополнитьТаблицуИзмененийПриемниками(ТаблицаИзменений, Параметры, РассчитываемыеИзмерения = Неопределено) Экспорт
	...
	Запрос.Текст = "
		...
		|	ТаблицаИзменений.ДатаНачалаПересчета КАК ДатаНачалаПересчета,
		...
		|	Изменения.ДатаНачалаПересчета                       КАК Период,
		|	Изменения.ПериодКонтроляИзменений                   КАК ПериодКонтроляИзменений,
		|	Изменения.ДатаНачалаПересчета                       КАК ДатаРегистратора,
		|	
		|	РасчетыСКлиентами.АналитикаУчетаПоПартнерам         КАК АналитикаУчетаПоПартнерам,
		|	РасчетыСКлиентами.ОбъектРасчетов                    КАК ОбъектРасчетов,
