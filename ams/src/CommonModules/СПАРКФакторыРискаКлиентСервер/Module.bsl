...
Функция НовыйРезультатФакторыРиска() Экспорт
	...
	Результат.Вставить(
		"ВидОшибки",
		ОбщегоНазначенияКлиентСервер.ПредопределенныйЭлемент("Перечисление.ВидыОшибокСПАРКРиски.ПустаяСсылка"));
...
Процедура ВывестиМногострочноеПредставлениеФакторовРиска(Форма, ФакторыРиска, ПредставлениеФакторов)
	...
	Если ФакторыРиска.ВидОшибки = ОбщегоНазначенияКлиентСервер.ПредопределенныйЭлемент("Перечисление.ВидыОшибокСПАРКРиски.НеизвестныйИНН") Тогда
		ПредставлениеФакторов.Добавить(
			Новый ФорматированнаяСтрока(
				...
				,
				"SPARK:NoInformation"));
	ИначеЕсли ФакторыРиска.ВидОшибки = ОбщегоНазначенияКлиентСервер.ПредопределенныйЭлемент("Перечисление.ВидыОшибокСПАРКРиски.НекорректныйИНН") Тогда
		ПредставлениеФакторов.Добавить(
			Новый ФорматированнаяСтрока(
				НСтр("ru='Нет информации о контрагенте'"),
				,
				...
				,
				"SPARK:NoInformation"));
	ИначеЕсли ФакторыРиска.ВидОшибки = ОбщегоНазначенияКлиентСервер.ПредопределенныйЭлемент("Перечисление.ВидыОшибокСПАРКРиски.НеизвестнаяОшибка") Тогда
		ПредставлениеФакторов.Добавить(
			Новый ФорматированнаяСтрока(
	...
	Если ЕстьОшибкаПодключения Тогда
		ПредставлениеФакторов = Новый Массив;
		Если ФакторыРиска.ВидОшибки = ОбщегоНазначенияКлиентСервер.ПредопределенныйЭлемент("Перечисление.ВидыОшибокСПАРКРиски.ТребуетсяОплатаИлиПревышенЛимит")
		...
		ИначеЕсли ФакторыРиска.ВидОшибки = ОбщегоНазначенияКлиентСервер.ПредопределенныйЭлемент("Перечисление.ВидыОшибокСПАРКРиски.ТребуетсяОплатаИлиПревышенЛимит")
			И ФакторыРиска.ДоступностьПодключенияТестовогоПериода = "Подключение" Тогда
			ПредставлениеФакторов.Добавить(
				НСтр("ru = 'Выполняется подключение сервиса.'"));
		ИначеЕсли ФакторыРиска.ВидОшибки = ОбщегоНазначенияКлиентСервер.ПредопределенныйЭлемент("Перечисление.ВидыОшибокСПАРКРиски.ЗапрещенДоступКИнтернетСервисам") Тогда
			ПредставлениеФакторов.Добавить(
				Новый ФорматированнаяСтрока(
...
Процедура ВывестиОбщиеФакторыРиска(ФакторыРиска, ПредставлениеФакторов, ЦветаСтилей)
	...
	Если ФакторыРиска.ВидКонтрагента = ОбщегоНазначенияКлиентСервер.ПредопределенныйЭлемент("Перечисление.ВидыКонтрагентовСПАРКРиски.ЮридическоеЛицо") Тогда
...
Процедура ВывестиКарточкуФакторыРиска(Форма, ФакторыРиска)
	...
	Если Форма.ВидКонтрагента = ОбщегоНазначенияКлиентСервер.ПредопределенныйЭлемент("Перечисление.ВидыКонтрагентовСПАРКРиски.ЮридическоеЛицо") Тогда
		ДанныеФакторыРиска = ФакторыРиска.ФакторыРискаЮридическогоЛица;
	Иначе
		ДанныеФакторыРиска = ФакторыРиска.ФакторыРискаФизическогоЛица;
	КонецЕсли;
