...
Процедура ЗаполнитьТаблицыШтрихкодов(ДокументОбъект, ПараметрыОкончанияСканирования, 
	...
	Если ПроверкаЗаверешенаСРасхождениями Тогда
		...
		|	ТаблицаШтрихкодыПлан.ЗначениеШтрихкода КАК ЗначениеШтрихкода,
		|	ТаблицаШтрихкодыПлан.ВидУпаковки КАК ВидУпаковки,
		|	ТаблицаШтрихкодыПлан.КоличествоПринято + ТаблицаШтрихкодыПлан.КоличествоНедостача + ТаблицаШтрихкодыПлан.КоличествоБрак КАК Количество
		|ИЗ
		|	ТаблицаШтрихкодыПлан КАК ТаблицаШтрихкодыПлан
		|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.Номенклатура КАК СправочникНоменклатура
		|		ПО ТаблицаШтрихкодыПлан.Номенклатура = СправочникНоменклатура.Ссылка
		|";
	...
	Если ПроверкаЗаверешенаСРасхождениями Тогда
		...
		Если ЭтоНовыйДокумент Тогда
			ВыборкаШтрихкодыПлан = Результат[9].Выбрать();
			Пока ВыборкаШтрихкодыПлан.Следующий() Цикл
				Если ВыборкаШтрихкодыПлан.ВидПродукции = ВидМаркируемойПродукцииОтбор Тогда
					Продолжить;
				КонецЕсли;
				НоваяСтрока = ДокументОбъект.ШтрихкодыУпаковокФактЭДО.Добавить();
				ЗаполнитьЗначенияСвойств(НоваяСтрока, ВыборкаШтрихкодыПлан);
			КонецЦикла;
		КонецЕсли;
