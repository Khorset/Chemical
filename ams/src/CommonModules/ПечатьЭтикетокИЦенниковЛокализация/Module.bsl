...
Процедура ПечатьЭтикетокИСМП(ТаблицаДанных, ТабличныйДокумент, СтруктураНастроек) Экспорт
	...
	ПредставленияШаблонов = ИнтеграцияИСМПКлиентСервер.ПредставленияШаблоновКодаМаркировки();
			...
			Если СтрокаТЧ.ЭтоКодМаркировки
				...
				Если ДанныеШаблонаЭтикеток <> Неопределено
					...
					Если ОшибкаПоШаблону = Неопределено Тогда
						ЭлементПредставленияШаблонов = ПредставленияШаблонов.НайтиПоЗначению(СтрокаТЧ.Шаблон);
						Если ЭлементПредставленияШаблонов = Неопределено Тогда
							ПредставлениеШаблона = Строка(СтрокаТЧ.Шаблон);
						Иначе
							ПредставлениеШаблона = ЭлементПредставленияШаблонов.Представление;
						КонецЕсли;
