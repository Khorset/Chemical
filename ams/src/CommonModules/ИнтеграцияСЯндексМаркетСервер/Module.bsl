...
Функция ПолучитьДанныеПокупателяПоЗаказуИзСервиса(УчетнаяЗапись, СхемаРаботы, Знач ИдентификаторЗаказа,
	...
	Если СхемаРаботы = Перечисления.СхемыРаботыТорговыхПлощадок.DBS Тогда
		...
		Если ОтветСервиса.Ошибка = Неопределено Тогда
			ЗаполнитьЗначенияСвойств(ДанныеПокупателяЗаказа, ОтветСервиса.Результат["result"]);
		Иначе
...
//   Булево - результат подтверждения заказа в сервисе.
// 
Функция ПодготовитьЗаказ(УчетнаяЗапись, ИдентификаторЗаказа, СоставЗаказа, СоставЗаказаИзменен, ТребуетсяУИН = Ложь, ТребуетсяМаркировка = Ложь, Отказ)
...
Процедура ПолучитьДанныеЗаказов(Знач УчетнаяЗапись, ТаблицаЗаказов, Знач Параметры, Отказ = Ложь)
	...
	ИдентификаторМагазина = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(УчетнаяЗапись, "ИдентификаторМагазина");
	Параметры.Фильтр.ИдентификаторыКампаний.Добавить(ИдентификаторМагазина);
...
Функция НовоеОтправлениеИзСервиса(ИдентификаторЗаказа)
	...
	НомерОтправления = СтрШаблон("%1-1", ЧислоВСтроку(ИдентификаторЗаказа));
...
//          *** estimated - Булево - риблизительная ли дата доставки.
...
Функция ВключенТестовыйРежимЗаказов(УчетнаяЗапись)
	...
	Результат = ДополнительныеНастройки.ТестовыйРежимЗаказовFBS; // Булево
	Возврат Результат;
...
//                 - Произвольный - результат выполнения метода API.
//     * Ошибка - Неопределено - при выполнении запроса ошибок не возникло.
//              - см. ИнтеграцияСМаркетплейсамиСервер.НоваяОшибка
//
Функция ВыполнитьЗапросВСервис(УчетнаяЗапись, ТекстЗапроса, ПараметрыЗапроса, ТелоЗапроса, HTTPМетод,
	...
	ОтветСервиса.Вставить("Ошибка", Неопределено);
	...
	Попытка
		...
		АдресРесурса = СтрЗаменить(ТекстЗапроса, "{businessId}", ДанныеУчетнойЗаписи.ИдентификаторКабинета);
		АдресРесурса = СтрЗаменить(АдресРесурса, "{campaignId}", ДанныеУчетнойЗаписи.ИдентификаторМагазина);
		...
		Если HTTPОтвет.КодСостояния = 200 Тогда
			Если СлужебныеПараметры.ПолучениеДвоичныхДанных Тогда
				ОтветСервиса.Результат = HTTPОтвет.ПолучитьТелоКакДвоичныеДанные();
			Иначе
				Если СлужебныеПараметры.ПрочитатьВСоответствие Тогда
					ОтветСервиса.Результат = Новый Соответствие;
				Иначе
					ОтветСервиса.Результат = Новый Структура;
				КонецЕсли;
		...
		Иначе
			...
			ОшибкаИзОтвета = ОписаниеОшибкиИзСервиса(ОжидаемыйСоставОтвета["errors"], АдресСервера, АдресРесурса);
			ОтветСервиса.Ошибка = ОшибкаИзОтвета;
			
			ЗаписьЖурналаРегистрации(
				СобытиеЖурналаРегистрации(),
				УровеньЖурналаРегистрации.Ошибка, ,
				УчетнаяЗапись,
				ОшибкаИзОтвета.ОписаниеОшибки);
		КонецЕсли;
	Исключение
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Отсутствует соединение с сервером %1 по причине: %2'",
		...
		ОтветСервиса.Ошибка = ИнтеграцияСМаркетплейсамиСервер.НоваяОшибка();
		ОтветСервиса.Ошибка.КодОшибки = ИнтеграцияСМаркетплейсамиСервер.КодыОшибок().ОшибкаПрочие;
		ОтветСервиса.Ошибка.ОписаниеОшибки = ТекстОшибки;
...
Процедура ЗаполнитьРезультатЗапросаПоОтветуСервиса(HTTPОтвет, Результат, ПоляТипаДата, ПрочитатьВСоответствие)
	...
	ДанныеОтвета = ИзJSON(ОтветСтрокой, ПоляТипаДата, ПрочитатьВСоответствие);
...
Функция ДанныеСледующейСтраницы(ОтветСервиса)
	...
	Если ЭтоСтруктура И Не ОтветСервиса.Свойство("paging") Тогда
		Возврат Результат;
	КонецЕсли;
	
	ДанныеСтраницы = ОтветСервиса["paging"];
	
	Если ЭтоСоответствие И ДанныеСтраницы = Неопределено Тогда
		Возврат Результат;
		
	ИначеЕсли ЭтоСтруктура И Не ДанныеСтраницы.Свойство("nextPageToken") Тогда
		Возврат Результат;
	КонецЕсли;
	
	ИдентификаторСтраницы = ДанныеСтраницы["nextPageToken"];
...
Процедура ЗаполнитьЗначенияСвойствПриемникаРекурсивно(Приемник, Источник)
	...
	Для Каждого КлючЗначение Из Приемник Цикл
		...
		ДанныеИсточника = Источник[ИмяСвойства];
