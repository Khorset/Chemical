...
Процедура ПриОпределенииЗапросаДляИСМП(
	...
	ИначеЕсли МетаданныеДокументаОснования = Метаданные.Документы.ПеремещениеТоваров Тогда
		...
		ИначеЕсли МетаданныеДокументаИСМП = Метаданные.Документы.ПеремещениеМеждуМОДИСМП Тогда
			...
			ДополнитьТекстЗапросаПолучениемАктуальныхДанныхРегистрацииОрганизацийВНалоговомОргане(ТекстЗапроса, МетаданныеДокументаОснования.Имя);
...
Процедура ДополнитьТекстЗапросаПолучениемАктуальныхДанныхРегистрацииОрганизацийВНалоговомОргане(ТекстЗапроса, ИмяДокумента)
	...
	ТекстЗапроса = ТекстЗапроса
		...
		+ СтрШаблон("
			...
			|	ТаблицаДокумента.СкладОтправитель.Подразделение               КАК ПодразделениеСкладаОтправителя,
			|	ТаблицаДокумента.СкладПолучатель.Подразделение                КАК ПодразделениеСкладаПолучателя,
			|	МАКСИМУМ(РегистрацииВНалоговомОрганеСкладаОтправителя.Период) КАК ДатаПоследнегоИзмененияКППОтправителя,
			|	МАКСИМУМ(РегистрацииВНалоговомОрганеСкладаПолучателя.Период)  КАК ДатаПоследнегоИзмененияКПППолучателя
			|ПОМЕСТИТЬ ДатыПоследнейРегистрацииВНалоговомОргане
			|ИЗ Документ.%1 КАК ТаблицаДокумента
			...
			|			И РегистрацииВНалоговомОрганеСкладаОтправителя.Период <= ТаблицаДокумента.Дата
			...
			|			И РегистрацииВНалоговомОрганеСкладаПолучателя.Период <= ТаблицаДокумента.Дата
			|ГДЕ
			|	ТаблицаДокумента.Ссылка В (&МассивДокументов)
			|СГРУППИРОВАТЬ ПО
			|	ТаблицаДокумента.Ссылка,
			|	ТаблицаДокумента.Организация,
			|	ТаблицаДокумента.СкладОтправитель.Подразделение,
			|	ТаблицаДокумента.СкладПолучатель.Подразделение
			|", ИмяДокумента)
		...
		+ СтрШаблон("
			|ВЫБРАТЬ
			|	ТаблицаДокумента.Ссылка          КАК Ссылка,
			|	ДанныеРегистрацииОтправителя.КПП КАК КППОтправителя,
			|	ДанныеРегистрацииПолучателя.КПП  КАК КПППолучателя
			|ПОМЕСТИТЬ РегистрацииОтправителяПолучателяВНалоговомОргане
			|ИЗ ДатыПоследнейРегистрацииВНалоговомОргане КАК ТаблицаДокумента
			|	ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.РегистрацииВНалоговомОргане КАК РегистрацииВНалоговомОрганеСкладаОтправителя
			...
			|		ПО ТаблицаДокумента.Организация = РегистрацииВНалоговомОрганеСкладаОтправителя.Организация
			|			И ТаблицаДокумента.ПодразделениеСкладаОтправителя = РегистрацииВНалоговомОрганеСкладаОтправителя.Подразделение
			|			И ТаблицаДокумента.ПодразделениеСкладаОтправителя <> ЗНАЧЕНИЕ(Справочник.СтруктураПредприятия.ПустаяСсылка)
			|			И ТаблицаДокумента.ДатаПоследнегоИзмененияКППОтправителя = РегистрацииВНалоговомОрганеСкладаОтправителя.Период
			|	ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.РегистрацииВНалоговомОргане КАК РегистрацииВНалоговомОрганеСкладаПолучателя
			...
			|		ПО ТаблицаДокумента.Организация = РегистрацииВНалоговомОрганеСкладаПолучателя.Организация
			|			И ТаблицаДокумента.ПодразделениеСкладаПолучателя = РегистрацииВНалоговомОрганеСкладаПолучателя.Подразделение
			|			И ТаблицаДокумента.ПодразделениеСкладаПолучателя <> ЗНАЧЕНИЕ(Справочник.СтруктураПредприятия.ПустаяСсылка)
			|			И ТаблицаДокумента.ДатаПоследнегоИзмененияКПППолучателя = РегистрацииВНалоговомОрганеСкладаПолучателя.Период")
		...
		+ СтрШаблон("
			...
			|ПОМЕСТИТЬ %2
			|ИЗ %3 КАК ТаблицаТовары
			|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.%1 КАК ТаблицаДокумента
			|		ПО ТаблицаТовары.Ссылка = ТаблицаДокумента.Ссылка
			|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрацииОтправителяПолучателяВНалоговомОргане КАК РегистрацииОтправителяПолучателяВНалоговомОргане
			|		ПО ТаблицаТовары.Ссылка = РегистрацииОтправителяПолучателяВНалоговомОргане.Ссылка
			|ГДЕ
			|	ТаблицаДокумента.Организация.ЮрФизЛицо = ЗНАЧЕНИЕ(Перечисление.ЮрФизЛицо.ИндивидуальныйПредприниматель)
			|		ИЛИ (ЕСТЬNULL(РегистрацииОтправителяПолучателяВНалоговомОргане.КППОтправителя, ТаблицаДокумента.Организация.КПП) <> ЕСТЬNULL(РегистрацииОтправителяПолучателяВНалоговомОргане.КПППолучателя, ТаблицаДокумента.ОрганизацияПолучатель.КПП))
			|", ИмяДокумента, ИмяВТ, ИмяЗамены);
