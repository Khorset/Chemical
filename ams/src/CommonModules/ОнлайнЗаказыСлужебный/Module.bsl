...
Функция ПараметрыОперацииОбновленияНастройки(НастройкаПодключения, РезультатОбработки)
	...
	ЗначенияРеквизитовОбъекта = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(
		НастройкаПодключения,
		"Используется,НастройкаПодключенияКОблачнойКассе");
...
//  СпособОплаты - Перечисления.СпособыОплатыОнлайнЗаказов - способ оплаты для которого выполняется получение токена.
//
...
//
Функция ОтозватьТокенНастройкиПодключения(ВидТокена, НастройкаПодключения, СпособОплаты) Экспорт
...
Функция ПубличныеДанныеЗаказа(URLЗаказа) Экспорт
	...
	Если ЗначениеЗаполнено(РезультатВызова.ИнформацияОбОшибке) Тогда
		ПубличныеДанныеЗаказа.ПараметрыShare.ИнформацияОбОшибке = РезультатВызова.ИнформацияОбОшибке;
	Иначе
		ДанныеЗаказа = РезультатВызова.ДанныеЗаказов.Получить(ИдентификаторЗаказа);
		Если ДанныеЗаказа <> Неопределено Тогда
			ПубличныеДанныеЗаказа.ПараметрыShare.URL = ДанныеЗаказа.URLShare;
		КонецЕсли;
	КонецЕсли;
...
Функция НовыйОписаниеПубличныхДанныхЗаказа()
...
Функция ВложенияURLShare(ДанныеВложений)
	...
	Для Каждого ДанныеВложения Из ДанныеВложений Цикл
		...
		Вложение.ДанныеФайла = ПолучитьИзВременногоХранилища(ДанныеВложения.Значение.АдресВоВременномХранилище);
...
Процедура ЗафиксироватьОбработкуЗаказа(
	...
	Попытка
		...
		ЗаписатьДанныеФискализации(ПараметрыФискализации, ДанныеФискализации);
		РегистрыСведений.ИдентификаторыОнлайнЗаказов.УстановитьОтложенноеПолучениеСтатуса(
			ДокументЗаказа,
			Ложь);
...
Процедура ЗаписатьДанныеФискализации(ПараметрыФискализации, ДанныеФискализации)
