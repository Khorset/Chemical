...
//  *ДокументОперации - ДокументСсылка - документ, который является источником платежной ссылки.
//  *ДокументОснование - ДокументСсылка - основание документа операции.
//  *Организация - СправочникСсылка.Организации - организация документа.
//  *ФормаОплаты - ПеречислениеСсылка.ФормыОплаты - форма оплаты документа.
...
Функция СуммыДокумента(ДокументОплаты, ДокументОснование) Экспорт
	...
	Если ТипЗнч(ДокументОснование) = Тип("СправочникСсылка.ДоговорыКонтрагентов") Тогда
		...
		ТекстЗапроса = 
			...
			|		ПО ЭтапыОплаты.Ссылка.ДокументОснование = ДоговорыКонтрагентов.Ссылка
			|			И ЭтапыОплаты.Ссылка = &ДокументОплаты
			|			И ДоговорыКонтрагентов.Ссылка = &ДокументОперации
			|
			|СГРУППИРОВАТЬ ПО
	...
	ИтоговаяВыборка =
		...
		|	ТаблицаСумм.СуммаСНДС КАК СуммаСНДС
		|ИЗ
		|	&ТаблицаСумм КАК ТаблицаСумм";
	
	Если ЭтоСчет Тогда
		ИтоговаяВыборка = СтрЗаменить(ИтоговаяВыборка, "&ТаблицаСумм", "СуммыЧастичнойОплаты");
	Иначе
...
Функция ТекстЗапросаСуммЧастичнойОплаты()
	...
	|	ЭтапыОплаты.СуммаПлатежа КАК СуммаСНДС,
	|	ВЫБОР
