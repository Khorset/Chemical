...
Функция РассчитатьЗаписиНДСПредъявленныйВосстановлениеПодВидДеятельности(ПараметрыРасчета)
	...
	|	ДД.РеализацияЭкспорт       КАК РеализацияЭкспорт,
	|	ДД.НаправлениеДеятельности КАК НаправлениеДеятельности,
	|	МАКСИМУМ(ДД.ОтнестиКПредыдущемуКварталу)      КАК ОтнестиКПредыдущемуКварталу,
	|	МАКСИМУМ(ДД.ИсправляемыйПериод)               КАК ИсправляемыйПериод,
	|	МАКСИМУМ(ДД.ДатаИсправленияРаспределенияНДС)  КАК ДатаИсправленияРаспределенияНДС,
	|	СУММА(ДД.СуммаБезНДС)      КАК СуммаБезНДС,
	|	СУММА(ДД.НДС)              КАК НДС,
	|	СУММА(ДД.НДСУпр)           КАК НДСУпр
...
Функция РассчитатьЗаписиНДСПредъявленныйПринятиеКВычету(ПараметрыРасчета)
	...
	ПартииСменыНалогообложения = РезультатЗапроса[3].Выгрузить();
	...
	ВыборкаОстатковНДСПредъявленныйПоСФ = РезультатЗапроса[4].Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам);
	НалоговаяБазаРеализации0ДляРаспределенияВычетаНДС = РезультатЗапроса[5].Выгрузить();
			...
			Для Каждого ПартияСменыНалогообложения Из СтрокиСменыНалогообложения Цикл
				Если Суммы.НДСКВычету * Знак > 0 Тогда
					Если НДСВосстановлен > 0 Тогда
						...
						ПартияСменыНалогообложения.НДСУпр = ПартияСменыНалогообложения.НДСУпр - Окр(ДоляДоступногоНДС * ПартияСменыНалогообложения.НДСУпр, 2);
						ПартияСменыНалогообложения.СуммаБезНДС = ПартияСменыНалогообложения.СуммаБезНДС - Окр(ДоляДоступногоНДС * ПартияСменыНалогообложения.СуммаБезНДС, 2);
					КонецЕсли;
					Если ПартияСменыНалогообложения.НДС * Знак > 0 Тогда
						...
						Иначе
							РаспределениеСуммыКВычету = ОбщегоНазначения.РаспределитьСуммуПропорциональноКоэффициентам(Мин(ПартияСменыНалогообложения.НДС, Суммы.НДСКВычету), Коэффициенты);
							Для Инд = 0 По СтрокиНалоговаяБазаРеализации0.Количество() - 1 Цикл
								ДоляДоступногоНДС = ?(ПартияСменыНалогообложения.НДС * Знак <> 0, РаспределениеСуммыКВычету[Инд] / ПартияСменыНалогообложения.НДС, 0);
								ДобавитьСтрокуВычетаНДС(ТаблицаНДСПредъявленныйПринятиеКВычету, Суммы, ДоляДоступногоНДС, ПартияСменыНалогообложения, СтрокиНалоговаяБазаРеализации0[Инд]);
							КонецЦикла;
						КонецЕсли;
					КонецЕсли;
				КонецЕсли;
				...
				Если НДСВосстановлен > 0 Тогда
					...
					Иначе
						...
						Для Инд = 0 По СтрокиНалоговаяБазаРеализации0.Количество() - 1 Цикл
							...
							ДобавитьСтрокуВычетаНДС(ТаблицаНДСПредъявленныйПринятиеКВычету, Суммы, ДоляДоступногоНДС, ВыборкаВРазрезеИзмеренийСФ, СтрокиНалоговаяБазаРеализации0[Инд], Истина, Истина);
						КонецЦикла;
					КонецЕсли;
					НДСВосстановлен = НДСВосстановлен - НДС;
				...
				Если Суммы.НДСКВычету * Знак > 0 Тогда
					...
					Иначе
						...
						Для Инд = 0 По СтрокиНалоговаяБазаРеализации0.Количество() - 1 Цикл
							...
							ДобавитьСтрокуВычетаНДС(ТаблицаНДСПредъявленныйПринятиеКВычету, Суммы, ДоляДоступногоНДС, ВыборкаВРазрезеИзмеренийСФ, СтрокиНалоговаяБазаРеализации0[Инд], Ложь, Истина);
						КонецЦикла;
					КонецЕсли;
				КонецЕсли;
			КонецЕсли;
...
Процедура ДобавитьСтрокуВычетаНДС(ТаблицаПринятиеКВычету,
	...
	Иначе
		НоваяСтрока.Регистратор = ПартияСменыНалогообложения.Регистратор;
		НоваяСтрока.Период      = ПартияСменыНалогообложения.Период;
...
Функция ВычетНДСАвансыВыданные(ПараметрыРасчета)
	...
	|			ВЫБРАТЬ
	|				РасширениеТаблицыДокументовОплат.Организация,
	|				РасширениеТаблицыДокументовОплат.ИсходныйДокументОплаты
	|			ИЗ
	|				РасширениеТаблицыДокументовОплат)) КАК ДД
	|	
	|	ОБЪЕДИНИТЬ ВСЕ
	|
	|	ВЫБРАТЬ
	...
	|			РасширениеТаблицыДокументовОплат КАК ВТ_ДокументыОплат
	|		ПО
	|			ДД.Организация = ВТ_ДокументыОплат.Организация
	|			И ДД.ДокументОплаты = ВТ_ДокументыОплат.ДокументОплаты
	|	ГДЕ
	|		ДД.Период МЕЖДУ &НачалоПериода И &КонецПериода
	|		И НЕ ДД.РегламентнаяОперация
	|		И ДД.Активность
