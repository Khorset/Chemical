...
Процедура ПолучитьИдентификаторОперацииСБП(Платеж)
	...
	Для Каждого Слово Из СписокСлов Цикл
		...
		Пока СтроковыеФункцииКлиентСервер.ЭтоРазделительСлов(КодСимвола(Слово, 1))
			Или СтроковыеФункцииКлиентСервер.ЭтоРазделительСлов(КодСимвола(Слово, 1), "_") Цикл
			Слово = Сред(Слово, 2, СтрДлина(Слово));
		КонецЦикла;
		
		Пока СтроковыеФункцииКлиентСервер.ЭтоРазделительСлов(КодСимвола(Слово, СтрДлина(Слово)))
