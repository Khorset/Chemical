...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	//!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~2
	мср.НастройкаФормыПечатьДокументов(ЭтотОбъект,Параметры.ИменаМакетов);
	Элементы.КнопкаПечатьКоманднаяПанель.Доступность = AMS_ВызовСервера.ПечатьРазрешена(Параметры.ПараметрКоманды,Параметры.ИменаМакетов);
	//!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~3
...
Процедура Печать(Команда)
	...
	Если ОбъектыПечати.Количество() > 0 Тогда 
		Если ЭтотОбъект.ИмяМакета = "СН" Тогда
			Событие = ПредопределенноеЗначение("Перечисление.AMS_СобытияРеализации.ПечатьСН");
		ИначеЕсли ЭтотОбъект.ИмяМакета = "ПечатьУПД" Тогда
			Событие = ПредопределенноеЗначение("Перечисление.AMS_СобытияРеализации.ПечатьУПД");
		ИначеЕсли ЭтотОбъект.ИмяМакета = "ПечатьТОРГ12" Тогда
			Событие = ПредопределенноеЗначение("Перечисление.AMS_СобытияРеализации.ПечатьТОРГ12");
		КонецЕсли;
		
		AMS_ВызовСервера.ЗаписатьСобытиеДокумента(ОбъектыПечати[0].Значение, Событие);
	КонецЕсли;
КонецПроцедуры
