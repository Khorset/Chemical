...
Функция Login(SN, UserName)
	...
	ДатаМобайл_ОбщийМодуль.ЗаписьЗначенийУзлаОбменаТСД(УзелПО, "ДатаПоследнейАвторизацииПользователя", ДатаМобайл_ОбщийМодуль.ПолучитьТекущуюДату());
...
Функция IsNewData(SN, UserName)
	...
	ДатаМобайл_ОбщийМодуль.ЗаписьЗначенийУзлаОбменаТСД(УзелПО, "ДатаПоследнейЗагрузкиСправочников", ДатаМобайл_ОбщийМодуль.ПолучитьТекущуюДату());	
...
Функция GetArtEgaisDateBottling(SN, UserName, DocOutID, ArtID, Barcode, PDFBarcode)
	...
	ОбъектСписка.DMDate.Добавить(ДатаМобайл_ОбщийМодуль.ПолучитьТекущуюДату());
...
Функция GetSelectedQtyGroupDocRows(SN, UserName, DocOutID, LastRequestDate)
	...
	ТекущаяДатаЗапроса = ДатаМобайл_ОбщийМодуль.ПолучитьТекущуюДату();
	...
	Если НЕ Шаблон.ГрупповаяРабота Тогда
		...
		Если НЕ ЗначениеЗаполнено(ДокДатаНачалаСбора) ИЛИ НЕ ЗначениеЗаполнено(ДокТСД) Тогда
			...
			Попытка
				...
				Объект.ДатаНачалаСбора = ДатаМобайл_ОбщийМодуль.ПолучитьТекущуюДату();
				Объект.ТСД = УзелПО;
...
Процедура ЗаполнитьОбъектШаблона(ОбъектШаблона,ШаблонСсылка)
	...
	|	ЛОЖЬ КАК DMUseOnlineArtsCatalog,
...
Функция OnWriteRecInsert(SN,UserName,DocOutID,СтрокаДокумента)
	...
	ЗначенияРеквизитовШаблон = Справочники.ДатаМобайл_ШаблоныДокументов.ПолучитьПараметрыШаблона(Шаблон,"ГрупповаяРабота,ОтсылатьГотовуюЗаписьНаСерверПриемка,ПриПревышенииЗаданияПриемка,ВидДокумента,БыстроеСканирование");
	Шаблон_ГрупповаяРабота = ЗначенияРеквизитовШаблон.ГрупповаяРабота;
...
Функция WriteDoc(SN, UserName, DocHead, DocRowsSelect,DocRowsInsert,Params)
	...
	Если Шаблон.БыстроеПеремещение <> 0 Тогда
		...
		Попытка
			...
			ИспользуетсяОрдернаяСхемаУСкладаПолучателя = Модуль_СкладыСервер.ИспользоватьОрдернуюСхемуПриОтгрузке(ТекущийСкладПолучатель, ДатаМобайл_ОбщийМодуль.ПолучитьТекущуюДату())
															Или Модуль_СкладыСервер.ИспользоватьОрдернуюСхемуПриПоступлении(ТекущийСкладПолучатель, ДатаМобайл_ОбщийМодуль.ПолучитьТекущуюДату());
			...
			ИспользуетсяОрдернаяСхемаУСкладаОтправителя = Модуль_СкладыСервер.ИспользоватьОрдернуюСхемуПриОтгрузке(ТекущийСкладОтправитель, ДатаМобайл_ОбщийМодуль.ПолучитьТекущуюДату())
															Или Модуль_СкладыСервер.ИспользоватьОрдернуюСхемуПриПоступлении(ТекущийСкладОтправитель, ДатаМобайл_ОбщийМодуль.ПолучитьТекущуюДату());
	...
	Если НЕ Шаблон.ГрупповаяРабота Тогда
		...
		Если isFinished Тогда
			Объект.ДатаЗавершенияСбора=ДатаМобайл_ОбщийМодуль.ПолучитьТекущуюДату();
		КонецЕсли;
	...
	Иначе
		...
		Если isFinished Тогда
			...
			Если (ДатаЗавершенияПоТСД = Неопределено) ИЛИ (ДатаЗавершенияПоТСД = Дата(1,1,1)) Тогда
				ДатаМобайл_ОбщийМодуль.ОбновитьЗаписьТСДГрупповогоДокумента(SN,СсылкаНаДок,ДатаМобайл_ОбщийМодуль.ПолучитьТекущуюДату());	
			КонецЕсли;
...
Функция CreateDoc(SN, UserName, DocOutID,TemplateID)
	...
	Если Рез.Следующий() Тогда
		...
		Объект.ДатаСоздания=ДатаМобайл_ОбщийМодуль.ПолучитьТекущуюДату();
		Объект.Источник = УзелПО;	
		Объект.ДатаНачалаСбора=ДатаМобайл_ОбщийМодуль.ПолучитьТекущуюДату();	
		Если НЕ (Объект.Шаблон.ГрупповаяРабота) Тогда
			Объект.ТСД=УзелПО;
		КонецЕсли;
	    Объект.Дата=ДатаМобайл_ОбщийМодуль.ПолучитьТекущуюДату();
...
Функция СоздатьДокумент(SN, UserName, DocOutID,TemplateID)
	...
	Если Рез.Следующий() Тогда
		...
		Док.ДатаСоздания=ДатаМобайл_ОбщийМодуль.ПолучитьТекущуюДату();
		Док.ДатаНачалаСбора=ДатаМобайл_ОбщийМодуль.ПолучитьТекущуюДату();
...
Функция SetDocWarehouse(SN, UserName, DocOutID, WarehouseID, Type)
	...
	Объект.ДатаИзмененияКлиента = ДатаМобайл_ОбщийМодуль.ПолучитьТекущуюДату();
	Попытка 
		Если Type = "1" Тогда
...
Функция SetRowPhoto(SN, UserName, DocID, ArtID, Barcode, Byte)
	...
	Если Шаблон.ИспользоватьФотофиксацию И Шаблон.ИспользоватьПодбор Тогда
		...
		Если СтрокиПодбор.Количество() > 0 Тогда
			...
			ПутьКФайлу = ПутьСохранения+Barcode+"_"+СтрЗаменить(Формат(ДатаМобайл_ОбщийМодуль.ПолучитьТекущуюДату(),"ДЛФ=T"),":","")+".jpg";				
...
Функция SetDocPhoto(SN, UserName, DocID, Byte)
	...
	ПутьКФайлу = Папка+DocID+"_"+СтрЗаменить(Формат(ДатаМобайл_ОбщийМодуль.ПолучитьТекущуюДату(),"ДЛФ=T"),":","")+".jpg";				
...
Функция SendPackToPrint(SN, UserName, DocID, Pack)
	...
	ЗаписьXML.ЗаписатьНачалоЭлемента("Footer");
		...
		Год = Строка(Формат(Год(ДатаМобайл_ОбщийМодуль.ПолучитьТекущуюДату()),"ЧГ="));
		Месяц = Строка(Месяц(ДатаМобайл_ОбщийМодуль.ПолучитьТекущуюДату()));
		День = Строка(День(ДатаМобайл_ОбщийМодуль.ПолучитьТекущуюДату()));
...
Процедура ВыполнитьПечатьШтрихкодаКороба(УзелПО,СтрокиРеквизитов, ЗаписьXML, Pack, ТЗСобранныеДанныеПодбор)
	...
	ЗаписатьЭлементXML(ЗаписьXML, "superdata", Формат(ДатаМобайл_ОбщийМодуль.ПолучитьТекущуюДату(), "ДП="));
...
Функция SendEgaisBlankABToPrint(SN, UserName, DocID, ArtID, Barcode, Alcocode, BlankA, BlankB)
	...
	ИмяФайла = СтрЗаменить(ДатаМобайл_ОбщийМодуль.ПолучитьТекущуюДату(),":","");
...
Функция ОтправитьСериюНаПечать(SN, UserName, DocID, СтруктураСтроки)
	...
	/////////////////////////////////////////////////
		...
		ДатаСтрока = Формат(ДатаМобайл_ОбщийМодуль.ПолучитьТекущуюДату(),"ДФ=dd.MM.yyyy");
...
Процедура OnArtScan_Select_Insert_Casual(УзелПО, Шаблон, ОбъектСписка, СсылкаНаДок, Запрос, Тип)
	...
	Модуль_ОбщегоНазначения = ДатаМобайл_ОбщийМодуль.ОбщийМодуль("ОбщегоНазначения");
	РеквизитыШаблона = Справочники.ДатаМобайл_ШаблоныДокументов.ПолучитьПараметрыШаблона(Шаблон,"ВидДокумента,ИмяТабличнойЧастиПодбор,ИмяТабличнойЧастиПриемка");
...
Процедура ПолучитьСтрокиДокументаМаркировка(УзелПО, Шаблон, ОбъектСписка, СсылкаНаДок)
	...
	Если Шаблон.ВидДокумента = "ПриобретениеТоваровУслуг" Тогда	
		...
		ДанныеНоменклатурыПоДаннымУПД = Модуль_ШтрихкодированиеИСМПСлужебный.ДанныеМаркируемойНоменклатурыПоДаннымУПД(СсылкаНаДок.ИсходныйДокумент);
	КонецЕсли;
...
Функция СобратьТовары(СписокТоваров, УзелПО, СписокХарактеристик = Неопределено, Шаблон = Неопределено)
	...
	ЗапросТоваров.УстановитьПараметр("ТекущаяДата", ДатаМобайл_ОбщийМодуль.ПолучитьТекущуюДату());
...
Функция ПроверитьЗаписи(ИмяТаблицы, УзелПО, СсылкаНаДок, СтрокаДокумента, Объект = Неопределено)
	...
	ЗначенияРеквизитовШаблон = Справочники.ДатаМобайл_ШаблоныДокументов.ПолучитьПараметрыШаблона(Шаблон,"ГрупповаяРабота,ИспользованиеУпаковочныхЛистовПодбор,БыстроеСканирование,ИспользоватьМаркировку,МаркировкаСоздаватьСправочникКМЕслиНеНайден");
	Шаблон_ГрупповаяРабота = ЗначенияРеквизитовШаблон.ГрупповаяРабота;
	Шаблон_ИспользованиеУпаковочныхЛистовПодбор = ЗначенияРеквизитовШаблон.ИспользованиеУпаковочныхЛистовПодбор;
	Шаблон_БыстроеСканирование = ЗначенияРеквизитовШаблон.БыстроеСканирование;
	Шаблон_ИспользоватьМаркировку = ЗначенияРеквизитовШаблон.ИспользоватьМаркировку;
...
Функция ПолучитьТоварыБезЗадания(УзелПО, СсылкаНаДок, КМ, GS1, ЗначениеШтрихкода)
	...
	ЗначенияРеквизитовШаблон = Справочники.ДатаМобайл_ШаблоныДокументов.ПолучитьПараметрыШаблона(Шаблон, "ВидДокумента");
	Шаблон_ВидДокумента = ЗначенияРеквизитовШаблон.ВидДокумента;
...
Функция ПреобразоватьвСистему(Число10, Система)
    ...
    Если Система > 36 Или Система < 2 Тогда
        ДатаМобайл_ОбщийМодуль.СообщитьПользователю("Выбранная система исчисления не поддерживается");
        Возврат -1;
    КонецЕсли;
