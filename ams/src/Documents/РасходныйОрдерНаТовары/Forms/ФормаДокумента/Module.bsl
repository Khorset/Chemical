...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	AMS_Сервер.ПередЗаписьюНаСервереРасходныйОрдерНаТовары(Отказ, ЭтотОбъект, ТекущийОбъект, ПараметрыЗаписи);
	//!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~1-3
...
//!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~2
	Если ТекущийСтатус = ПредопределенноеЗначение("Перечисление.СтатусыРасходныхОрдеров.Отгружен") Тогда
		Объект.Дата = Объект.ДатаОтгрузки;
	КонецЕсли;		
КонецПроцедуры
...
&НаКлиенте
Процедура Подключаемый_амсПриИзменении_ТоварыНоменклатура(Элемент)
	Стр = Элементы._Товары.ТекущиеДанные;
	
	Стр.Количество = амсПолучитьКоличество(Стр.Номенклатура);
	Стр.Вес = амсПолучитьВес(Стр.Номенклатура,Стр.Количество);
	
	амсРаспределитьВесПоМестамПогрузки();
КонецПроцедуры

&НаКлиенте
Процедура Подключаемый_амсПриИзменении_ТоварыКоличество(Элемент)
	Стр = Элементы._Товары.ТекущиеДанные;
...
&НаКлиенте
Процедура Подключаемый_амсНачалоВыбора_Работа(Элемент,ДанныеВыбора,СтандартнаяОбработка)
	Стр = Элементы._Работы.ТекущиеДанные;
	
	СписокВыбора = Элементы._РаботыРабота.СписокВыбора;
	
	СписокВыбора.Очистить();

	Если Стр.МестоПогрузки = ОбщегоНазначенияКлиент.ПредопределенныйЭлемент("Перечисление._МестаПогрузки.Основной") Тогда
		СписокВыбора.Добавить(ОбщегоНазначенияКлиент.ПредопределенныйЭлемент("Справочник._Работы.СкладКладовщик"));
		СписокВыбора.Добавить(ОбщегоНазначенияКлиент.ПредопределенныйЭлемент("Справочник._Работы.СкладКомплектовщик"));
		СписокВыбора.Добавить(ОбщегоНазначенияКлиент.ПредопределенныйЭлемент("Справочник._Работы.СкладОбмотчик"));
		СписокВыбора.Добавить(ОбщегоНазначенияКлиент.ПредопределенныйЭлемент("Справочник._Работы.СкладГрузчикВМашине"));
		СписокВыбора.Добавить(ОбщегоНазначенияКлиент.ПредопределенныйЭлемент("Справочник._Работы.СкладВодитель"));
	Иначе
		СписокВыбора.Добавить(ОбщегоНазначенияКлиент.ПредопределенныйЭлемент("Справочник._Работы.СкладКладовщикВерхний"));
		СписокВыбора.Добавить(ОбщегоНазначенияКлиент.ПредопределенныйЭлемент("Справочник._Работы.СкладКомплектовщикВерхний"));
		СписокВыбора.Добавить(ОбщегоНазначенияКлиент.ПредопределенныйЭлемент("Справочник._Работы.СкладОбмотчикВерхний"));
		СписокВыбора.Добавить(ОбщегоНазначенияКлиент.ПредопределенныйЭлемент("Справочник._Работы.СкладГрузчикВМашинеВерхний"));
		СписокВыбора.Добавить(ОбщегоНазначенияКлиент.ПредопределенныйЭлемент("Справочник._Работы.СкладВодительВерхний"));
	КонецЕсли;
КонецПроцедуры

&НаСервере
Функция амсПолучитьКоличество(Номенклатура)
	Количество = Объект.ТоварыПоРаспоряжениям.НайтиСтроки(Новый Структура("Номенклатура",Номенклатура))[0].Количество;
	
	//@skip-check bsl-legacy-check-string-literal
	Кфц = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Номенклатура, "КоэффициентЕдиницыДляОтчетов");
	
	Возврат Количество / Кфц;
КонецФункции

&НаСервере
Процедура амсРаспределитьВесПоМестамПогрузки()
	Распоряжение = Объект.ТоварыПоРаспоряжениям[0].Распоряжение;
	
	//@skip-check bsl-legacy-check-string-literal
	ВесОбщий = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Распоряжение, "AMS_Вес"); 
	
	Объект._ВесВерхний = Объект._Товары.Итог("Вес") / 1000;
	Объект._ВесОсновной = ВесОбщий - Объект._ВесВерхний;
КонецПроцедуры

&НаСервереБезКонтекста
Функция амсПолучитьВес(Номенклатура,Количество)
	//@skip-check bsl-legacy-check-string-literal
	Упаковка = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Номенклатура, "_Упаковка");
	
	//@skip-check bsl-legacy-check-string-literal
	ВесУпаковки = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Упаковка, "Вес");
	
	Возврат Количество * ВесУпаковки;
КонецФункции
