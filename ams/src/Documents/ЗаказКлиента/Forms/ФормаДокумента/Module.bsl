...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	//!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~2
	Если Параметры.Ключ.Пустая() И Объект.AMS_ID > 0 Тогда
...
Процедура ПартнерПриИзменении(Элемент)
	...
	Если ЗначениеЗаполнено(ПравилоПродажи) Тогда
		...
		AMS_УстановитьПравилаПродажи();
	Иначе
		Объект.AMS_ПравилоПродажи = ПредопределенноеЗначение("Справочник.AMS_ПравилаПродаж.ПустаяСсылка"); 
	КонецЕсли;
	AMS_УправлениеФормойЗаказКлиента();
КонецПроцедуры
...
Процедура ТоварыНоменклатураПриИзменении(Элемент)
	...
	//!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~13
	ТекущаяСтрока.Упаковка = ОбщегоНазначенияУТВызовСервера.ЗначениеРеквизитаОбъекта(ТекущаяСтрока.Номенклатура, "_Упаковка");
	//!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~14
...
Процедура КонтрагентПриИзмененииСервер()
	...
	РозничныеПродажиЛокализация.ПриИзмененииЭлемента(ЭтотОбъект, "Контрагент", Неопределено);
...
Процедура Подключаемый_ВыполнитьКоманду(Команда)
	Если Команда.Имя = "ПодменюСоздатьНаОснованииОбычное__1" Тогда
		Отказ = Ложь;
		AMS_ВызовСервера.ПроверитьВозможностьСозданияРеализацииНаОснованииЗаказаКлиента(Объект.Ссылка, Отказ);
		Если Отказ Тогда
			Возврат;
		КонецЕсли;
	КонецЕсли;
...
Процедура ПроверитьНаличиеСкидокОплатБонуснымиБалламиЛокализация()
	...
	РезультатПроверки = РозничныеПродажиЛокализация.ПроверитьНаличиеСкидокОплатБонуснымиБаллами(ЭтотОбъект, "Товары", Истина, Истина);
...
#Область AMS_СлужебныеПроцедурыИФункции
...
Процедура AMS_ПодборТоваровЗавершение(Результат, ДополнительныеПераметры) Экспорт
	...
	Если Объект.AMS_ТранспортныеУслуги Тогда 
		...
		Стр.Содержание               = "Транспортные услуги";
		Стр.ВариантОбеспечения       = ПредопределенноеЗначение("Перечисление.ВариантыОбеспечения.НеТребуется");
		Стр.ВариантОформленияПродажи = ПредопределенноеЗначение("Перечисление.ВариантыОформленияПродажи.АктВыполненныхРабот");
		Стр.Подразделение            = Объект.Подразделение;
		Стр.ПерераспределятьЗапасы   = Истина;
	КонецЕсли;
...
&НаКлиенте
Процедура AMS_ДатаОтгрузкиПриИзменении()
	Если Объект.Ссылка.Пустая() Или AMS_ВызовСервера.ЭтоKhors() Тогда
		Возврат;
	КонецЕсли;
	
	ДатаСегодня          = НачалоДня(AMS_ВызовСервера.ПолучитьРабочийДень(0,,,Истина));
	ДатаОтгрузки         = НачалоДня(Объект.ДатаОтгрузки);
	ДатаРеализации       = НачалоДня(AMS_ВызовСервера.ПолучитьРабочийДень(1,,,Истина));
	ДатаОтгрузкиПоСсылке = ОбщегоНазначенияУТВызовСервера.ЗначениеРеквизитаОбъекта(Объект.Ссылка, "ДатаОтгрузки");
	ДанныеРеализации     = AMS_ПолучитьРеализациюПоНомеруЗаказКлиента(Объект.Номер);
	
	Распоряжения = Новый Массив;
	Распоряжения.Добавить(Объект.Ссылка);
	Если Не ДанныеРеализации = Неопределено Тогда
		Распоряжения.Добавить(ДанныеРеализации.Реализация);
	КонецЕсли;
	
	Если AMS_ВызовСервера.ВключенВЛогитическийЛист(Распоряжения) Тогда //Отказать - есть логистический лист
		Текст = "Документ включен в логистический лист. Изменение даты отгрузки запрещено.";
		ПоказатьПредупреждение(, Текст, 10);
		Объект.ДатаОтгрузки = ДатаОтгрузкиПоСсылке;
	...
	Если ДатаОтгрузки < ДатаСегодня Тогда //вчера
		...
		Текст = СтрШаблон(Текст, Формат(ДатаСегодня, "ДЛФ=Д"));
		ПоказатьПредупреждение(, Текст, 10);
		Объект.ДатаОтгрузки = ДатаОтгрузкиПоСсылке;
		Модифицированность = Ложь;
		Возврат;
	...
	Если Не ДанныеРеализации = Неопределено Тогда
		Если ДатаОтгрузки <= ДатаРеализации Тогда //Изменить дату реализации
			Оповещение = Новый ОписаниеОповещения("AMS_ИзменитьДатуРеализацииЗавершение", ЭтотОбъект, ДанныеРеализации);
			Текст = "Дата документа ""Реализация товаров и услуг"" № %1 от %2 г. будет изменена. Продожить?";
			Текст = СтрШаблон(Текст, ПрефиксацияОбъектовКлиентСервер.НомерНаПечать(ДанныеРеализации.Номер), Формат(ДанныеРеализации.Дата, "ДЛФ=Д"));
			ПоказатьВопрос(Оповещение, Текст, РежимДиалогаВопрос.ДаНет, 0, КодВозвратаДиалога.Нет);

		Иначе //послезавтра
			Оповещение = Новый ОписаниеОповещения("AMS_УдалитьРеализациюЗавершение", ЭтотОбъект, ДанныеРеализации);
			Текст = "Документ ""Реализация товаров и услуг"" № %1 от %2 г. будет удален. Продожить?";
			Текст = СтрШаблон(Текст, ПрефиксацияОбъектовКлиентСервер.НомерНаПечать(ДанныеРеализации.Номер), Формат(ДанныеРеализации.Дата, "ДЛФ=Д"));
			ПоказатьВопрос(Оповещение, Текст, РежимДиалогаВопрос.ДаНет, 0, КодВозвратаДиалога.Нет);
		КонецЕсли;
	КонецЕсли;
...
&НаКлиенте
Процедура AMS_ИзменитьДатуРеализацииЗавершение(Результат, ДополнительныеПараметры) Экспорт
	Если Результат = КодВозвратаДиалога.Да Тогда
		Отказ = Ложь;
		Реализация = Неопределено;
		Если ДополнительныеПараметры.Свойство("Реализация", Реализация) Тогда
			AMS_ИзменитьДатуРеализацииЗавершениеНаСервере(Реализация, Объект.ДатаОтгрузки, Отказ);

			Модифицированность = Ложь;
			
			Если Отказ Тогда
				ОбщегоНазначенияКлиент.СообщитьПользователю("Изменить дату отгрузки не удалось",,,,Отказ);
			Иначе
				Оповестить("Запись_ЗаказКлиента",,Объект.Ссылка);
				Закрыть();
			КонецЕсли;
		КонецЕсли;
		
	Иначе
		Объект.ДатаОтгрузки = ДополнительныеПараметры.Дата;
		Модифицированность = Ложь;
	КонецЕсли;
КонецПроцедуры

&НаСервере
Процедура AMS_ИзменитьДатуРеализацииЗавершениеНаСервере(Реализация, ДатаОтгрузки, Отказ)
	НачатьТранзакцию();

	Попытка
		БлокировкаДанных = Новый БлокировкаДанных;
		ЭлементБлокировкиДанных = БлокировкаДанных.Добавить("Документ.РеализацияТоваровУслуг");
		ЭлементБлокировкиДанных.УстановитьЗначение("Ссылка", Реализация);
		ЭлементБлокировкиДанных.Режим = РежимБлокировкиДанных.Исключительный;
		БлокировкаДанных.Заблокировать();

		РеализацияОбъект = Реализация.ПолучитьОбъект();
		РеализацияОбъект.Дата = ДатаОтгрузки;
	...
	Исключение
		...
		ДатаОтгрузкиПоСсылке = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Объект.Ссылка, "ДатаОтгрузки");
		Объект.ДатаОтгрузки = ДатаОтгрузкиПоСсылке;
...
&НаКлиенте
Процедура AMS_УдалитьРеализациюЗавершение(Результат, ДополнительныеПараметры) Экспорт
	Если Результат = КодВозвратаДиалога.Да Тогда
		Отказ = Ложь;
		Реализация = Неопределено;
		Если ДополнительныеПараметры.Свойство("Реализация", Реализация) Тогда
			AMS_УдалитьРеализациюЗавершениеНаСервере(Реализация, Отказ);
...
&НаСервере
Процедура AMS_УдалитьРеализациюЗавершениеНаСервере(Реализация, Отказ)
	НачатьТранзакцию();
	...
	Попытка
		...
		ЭлементБлокировкиДанных = БлокировкаДанных.Добавить("Документ.РеализацияТоваровУслуг");
		ЭлементБлокировкиДанных.УстановитьЗначение("Ссылка", Реализация);
		ЭлементБлокировкиДанных = БлокировкаДанных.Добавить("Документ.ЗаказКлиента");
		...
		ЗаказОбъект.ДатаОтгрузки = Объект.ДатаОтгрузки;
...
&НаСервереБезКонтекста
Функция AMS_ПолучитьРеализациюПоНомеруЗаказКлиента(AMS_ЗаказКлиентаНомер) 
	Запрос = Новый Запрос;
...
Процедура AMS_ЗаполнитьВесОбъемКолПалет()
	AMS_Сервер.ЗаполнитьВесОбъемКолПалетЗаказКлиента(Объект);
КонецПроцедуры

&НаСервере
Процедура AMS_УправлениеФормойЗаказКлиента()
	AMS_СобытияФорм.УправлениеФормойЗаказКлиента(ЭтотОбъект);
КонецПроцедуры

&НаСервере
Процедура AMS_УправлениеРеквизитамиДоставки()
	AMS_СобытияФорм.УправлениеРеквизитамиДоставкиAMS(ЭтотОбъект, "ЗаказКлиента");
КонецПроцедуры
