...
Процедура СтатусПриИзменении(Элемент)
	...
	AMS_Клиент.ПриИзмененииРеквизитаПересчетТоваровСтатус(Объект, ЭтотОбъект._мСтатус);
	//!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~1-3
...
&НаКлиенте
Процедура Подключаемый_амсНачалоВыбора_Статус(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	ЭтотОбъект._мСтатус = Объект.Статус;
КонецПроцедуры

&НаКлиенте
Процедура Подключаемый_амсКоманда_ВнесениеРезультатов(Команда)
	ОткрытьФорму("Документ.ПересчетТоваров.Форма.AMS_ВнесениеРезультатов", амсПолучитьПараметры(),,,,,
		Новый ОписаниеОповещения("амсВнесениеРезультатовЗавершение", ЭтотОбъект),
		РежимОткрытияОкнаФормы.БлокироватьВесьИнтерфейс);
КонецПроцедуры

&НаСервере
Функция амсПолучитьПараметры()
	МассивСтрок = Новый Массив();
	...
	Возврат Новый Структура("Статус, Товары", Объект.Статус, МассивСтрок);
КонецФункции 

&НаКлиенте
Процедура амсВнесениеРезультатовЗавершение(Результат,ДополнительныеПераметры) Экспорт
	...
	Для Каждого Стр Из Результат Цикл
		...
		НовСтр.Артикул = ОбщегоНазначенияУТВызовСервера.ЗначениеРеквизитаОбъекта(НовСтр.Номенклатура, "Артикул");
		НовСтр.ТипИзмеряемойВеличины = ПредопределенноеЗначение("Перечисление.ТипыИзмеряемыхВеличин.КоличествоШтук");
		НовСтр.ТипНоменклатуры = ОбщегоНазначенияУТВызовСервера.ЗначениеРеквизитаОбъекта(НовСтр.Номенклатура, "ТипНоменклатуры");
		НовСтр.Упаковка = ПредопределенноеЗначение("Справочник.УпаковкиЕдиницыИзмерения.ПустаяСсылка");
	КонецЦикла;
