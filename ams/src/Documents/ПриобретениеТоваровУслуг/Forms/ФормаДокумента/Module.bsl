...
Процедура ПартнерПриИзменении(Элемент)
	...
	//!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~2
	амсУстановитьСкладПолученияТовара();
КонецПроцедуры
...
Функция ПроверитьПараметрыНоменклатурыСервер()
	...
	Документы.ПриобретениеТоваровУслуг.ПроверитьТипыНоменклатуры(Объект.Организация, Объект.Договор,
		Объект.Товары.Выгрузить().ВыгрузитьКолонку("Номенклатура"), Отказ);
...
&НаКлиенте
Процедура Подключаемый_амсПечать_АктПереводаНаименований(Команда)
	...
	УправлениеПечатьюКлиент.ВыполнитьКомандуПечати("Обработка.AMS_Продажи","АктПереводаНаименований", Массив, Неопределено, Новый Структура());
КонецПроцедуры

&НаСервере
Процедура амсУстановитьСкладПолученияТовара()
	мср.УстановитьСкладПолученияТовара(Объект);
	...
	Если Не РольДоступна("_Бухгалтер") Тогда
		Объект.Менеджер        = Объект.Автор;
		Объект.Принял          = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Объект.Автор, "ФизическоеЛицо");
		Объект.ПринялДолжность = "Кладовщик";
	КонецЕсли;
КонецПроцедуры		

&НаКлиенте
Процедура амсВыборПоставщикаЗавершение(Результат,ДополнительныеПераметры) Экспорт
	Если Результат = Неопределено Тогда
		Возврат;
	КонецЕсли;
	...
	Для каждого Стр Из Табл Цикл
		...
		НовСтр.НоменклатураПартнера = Стр.НоменклатураПартнера; 
