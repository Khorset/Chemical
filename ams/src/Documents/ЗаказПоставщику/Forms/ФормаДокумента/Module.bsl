...
&НаКлиенте
Процедура Подключаемый_амсКоманда_ВыборПоставщика()
	ОткрытьФорму("ОбщаяФорма.AMS_ВыборПоставщика",
		Новый Структура("Дата,Склад", Объект.Дата, Объект.Склад),,,,,
		Новый ОписаниеОповещения("амсВыборПоставщикаЗавершение",ЭтотОбъект),
...
Процедура амсВыборПоставщикаЗавершение(Результат, ДополнительныеПераметры) Экспорт
	...
	Для каждого Стр Из Табл Цикл
		...
		НовСтр._КоличествоЗаказано  = Стр.Количество;
		НовСтр.Цена                 = Стр.Цена;
		
		Если Объект.Статус = ОбщегоНазначенияКлиент.ПредопределенныйЭлемент("Перечисление.СтатусыЗаказовПоставщикам.НеСогласован") Тогда
			НовСтр._Остаток = Стр.Остаток;
		КонецЕсли;
	КонецЦикла;
...
Процедура AMS_УправлениеРеквизитамиДоставки()
	AMS_СобытияФорм.УправлениеРеквизитамиДоставкиAMS(ЭтотОбъект, "ЗаказПоставщику");
КонецПроцедуры
