...
Процедура ТоварыПриОкончанииРедактированияНаСервереЛокализация(ИдентификаторСтроки, КэшированныеЗначения)
	...
	Если КэшированныеЗначения.Свойство("ОбнулитьЦенуДляТабачнойПродукции") И КэшированныеЗначения.ОбнулитьЦенуДляТабачнойПродукции = Истина Тогда
		ТекущиеДанные = Объект.Товары.НайтиПоИдентификатору(Элементы.Товары.ТекущаяСтрока);
		Если НоменклатураСОсобенностьюУчета(ТекущиеДанные.Номенклатура, Перечисления.ОсобенностиУчетаНоменклатуры.ТабачнаяПродукция).Количество() Тогда
...
Процедура ПроверитьЦеныПоПрайсЛистуЛокализация()
	...
	Если ЗначениеЗаполнено(Объект.Ссылка) Тогда
		...
		Для Каждого СтрокаСЦенойПрайса Из ОбрабатываемыеСтроки Цикл
			...
			//++ Локализация
			Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(СтрокаСЦенойПрайса, "ВидПродукцииИС") Тогда
			
				ВидыПродукцииСМРЦ = РозничныеПродажиКлиентСерверЛокализация.ВидыПродукцииСМРЦ();
				ТабачнойПродукцииТолькоПоМРЦ = РозничныеПродажиКлиентСерверЛокализация
					.РозничныеПродажиТабачнойПродукцииТолькоПоМРЦ(ЭтотОбъект);
				
				Если ВидыПродукцииСМРЦ.Найти(СтрокаСЦенойПрайса.ВидПродукцииИС) <> Неопределено
					И ТабачнойПродукцииТолькоПоМРЦ Тогда
					Продолжить;
				КонецЕсли;
...
&НаСервере
Функция НоменклатураСОсобенностьюУчета(Номенклатура, ОсобенностьУчета)
	...
	ИначеЕсли ТипЗнч(Номенклатура) = Тип("Массив") Тогда
		МассивНоменклатуры = Новый Массив(Номенклатура);
	ИначеЕсли ТипЗнч(Номенклатура) = Тип("ТаблицаЗначений") Тогда
		МассивНоменклатуры = Номенклатура.ВыгрузитьКолонку("Номенклатура");
	Иначе
		Возврат Результат;
	КонецЕсли;
	...
	Запрос.Текст = "ВЫБРАТЬ РАЗЛИЧНЫЕ
	               ...
	               |	И Номенклатура.ВидНоменклатуры.ОсобенностьУчета = &ОсобенностьУчета";
	Запрос.УстановитьПараметр("Номенклатура", МассивНоменклатуры);
	Запрос.УстановитьПараметр("ОсобенностьУчета", ОсобенностьУчета);
	Выборка = Запрос.Выполнить().Выбрать();
	Пока Выборка.Следующий() Цикл
		Результат.Вставить(Выборка.Номенклатура, ОсобенностьУчета);
	КонецЦикла;
