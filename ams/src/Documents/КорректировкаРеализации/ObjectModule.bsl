...
Процедура ЗаполнитьДопКолонкиВидовЗапасов(ВидыЗапасов, МассивВариантовОтражения, ЗаполнятьВидЗапасовОтгрузки = Ложь, ЗаполнятьСебестоимость = Ложь) Экспорт
	...
	Для Каждого СтрокаРасхождений Из Расхождения Цикл
		...
		Если УчитыватьСтроку Тогда
			...
			Если КорректировкаРеализации2_5 Тогда 
				Если НачалоГода(Дата) = НачалоГода(ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДокументОснование, "Дата")) Тогда
					СтрокаТоваров.НаДоходыРасходы = Ложь;
				Иначе 
					СтрокаТоваров.НаДоходыРасходы = ОтразитьВПрибылиУбыткахПрошлыхЛет 
						И КорректироватьНалоговуюБазуПрошлогоПериода;
				КонецЕсли;
			Иначе 	
