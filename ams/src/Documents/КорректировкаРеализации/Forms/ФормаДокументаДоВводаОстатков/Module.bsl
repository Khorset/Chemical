...
Функция ПолучитьДоступныВариантыОтраженияДляСтроки(СтрокаТаблицы, КорректировкаУслугПрочихАктивов, КорректировкаПрошлогоПериода, КорректировкаРеализации2_5)
	...
	ИначеЕсли КорректировкаУслугПрочихАктивов Тогда
		...
		Если КорректировкаПрошлогоПериода Тогда
			...
			Если ДобавлятьСписатьНаРасходы И ДанныеПоРасхождению.СуммаСНДСРасхождение < 0 Тогда
				СписокВыбора.Добавить(ПредопределенноеЗначение("Перечисление.ВариантыОтраженияКорректировокРеализаций.СписатьНаРасходы"));
			ИначеЕсли ДобавлятьОтразитьНаПрочихДоходах И ДанныеПоРасхождению.СуммаСНДСРасхождение >= 0 Тогда
				СписокВыбора.Добавить(ПредопределенноеЗначение("Перечисление.ВариантыОтраженияКорректировокРеализаций.ОтразитьНаПрочихДоходах"));
			КонецЕсли;
			
		ИначеЕсли ДобавлятьОтразитьНаПрочихДоходах Тогда
			СписокВыбора.Добавить(ПредопределенноеЗначение("Перечисление.ВариантыОтраженияКорректировокРеализаций.ОтразитьНаПрочихДоходах"));
		КонецЕсли;
