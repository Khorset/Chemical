...
Функция ПроверитьОтключитьНастроитьФормуПоПорче()
	...
	ПоискСтрокСПорчей = Объект.Товары.НайтиСтроки(
		Новый Структура("Порча, НоменклатураОприходование",
			Ложь,
			Справочники.Номенклатура.ПустаяСсылка()));
	
	ЕстьСтрокиСПорчей = ПоискСтрокСПорчей.Количество() <> Объект.Товары.Количество();
