...
Процедура ИзменитьТабличнуюЧастьПоРезультатамПроверки(ВозвращаемыеПараметры, КэшированныеЗначения)
	...
	ЗаказыКлиентСервер.ДобавитьДействиеЗаполнитьПризнакРасхождениеЗаказ(ОбрабатываемыеСтрокиДобавленныеСтроки, Объект.ОтгрузкаПоЗаказам);
...
Процедура Подключаемый_AMS_ПриИзменении_ПартнерДопИнфо(Элемент)
	Если ЗначениеЗаполнено(Объект.Партнер) Тогда 
		...
		AMS_ВызовСервера.ИзменитьПартнерДопИнфо(Объект.Партнер, AMS_ПартнерДопИнфо);
	КонецЕсли;
КонецПроцедуры
...
//	AMS_ПодборТоваровЗавершениеНаСервере(Результат);
//
//	амсЗаполнитьВесОбъемКолПалет(); 
//	
...
Процедура AMS_УправлениеРеквизитамиДоставки()
	AMS_СобытияФорм.УправлениеРеквизитамиДоставкиAMS(ЭтотОбъект, "ОтгрузкаТоваровКлиенту");
КонецПроцедуры
