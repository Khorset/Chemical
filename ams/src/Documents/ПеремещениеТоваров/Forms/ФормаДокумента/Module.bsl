...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	//мср.ПередЗаписьюНаСервереПеремещениеТоваров(Отказ, ТекущийОбъект, ПараметрыЗаписи);
	//!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~1-6
...
Процедура СтатусПриИзменении(Элемент)
	//AMS_Клиент.ПриИзмененииРеквизитаПеремещениеТоваровСтатусИлиДата(Объект);
	//
	//ДатаНачалаОрдернойСхемы	= ОбщегоНазначенияУТВызовСервера.ЗначениеРеквизитаОбъекта(Объект.СкладОтправитель, "ДатаНачалаОрдернойСхемыПриОтраженииИзлишковНедостач");
	//	
	//Если Объект.Дата < ДатаНачалаОрдернойСхемы Тогда
	//	Возврат;
	//КонецЕсли;
	//!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~4
...
&НаКлиенте
Процедура Подключаемый_AMS_Команда_ЗаполнитьПродукцию(Команда)
	Если Не ЗначениеЗаполнено(Объект.СкладПолучатель) Тогда
		ОчиститьСообщения();
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Поле ""Склад-получатель"" не заполнено'"), Объект.Ссылка, "Объект.СкладПолучатель");
		Возврат;
	
	ИначеЕсли Объект.СкладОтправитель = AMS_ВызовСервера.Цех4() 
		И Не (Объект.СкладПолучатель = AMS_ВызовСервера.Цех1() Или Объект.СкладПолучатель = AMS_ВызовСервера.Цех2()) Тогда
		ОчиститьСообщения();
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Поле ""Склад-получатель"" заполнено не верно'"), Объект.Ссылка, "Объект.СкладПолучатель");
		Возврат;
	КонецЕсли;
	
	AMS_ЗаполнитьПродукцию(Объект);
	Модифицированность = Истина;
КонецПроцедуры

&НаКлиенте
Процедура AMS_ЗаполнитьПродукцию(Объект)
	Если Объект.Товары.Количество() = 0 Тогда
		Если Объект.СкладОтправитель = AMS_ВызовСервера.Цех4() Тогда
			AMS_ЗаполнитьФлаконыДляПеремещения(Объект);
		Иначе
			AMS_ЗаполнитьТоварыДляПеремещения(Объект);
		КонецЕсли;
	Иначе
		ДополнительныеПараметры = Новый Структура;
		ДополнительныеПараметры.Вставить("Объект", Объект);
		ТекстВопроса = НСтр("ru='Таблица товары будет очищена. Продолжить?'");
		ПоказатьВопрос(Новый ОписаниеОповещения("AMS_ВопросПользователюЗаполнитьПродукциюЗавершение", ЭтотОбъект,
			ДополнительныеПараметры), ТекстВопроса, РежимДиалогаВопрос.ДаНет);
	КонецЕсли;
КонецПроцедуры

&НаКлиенте
Процедура AMS_ВопросПользователюЗаполнитьПродукциюЗавершение(Ответ, ДополнительныеПараметры) Экспорт
	Если Ответ = КодВозвратаДиалога.Да Тогда
		AMS_ЗаполнитьТоварыДляПеремещения(ДополнительныеПараметры.Объект);
	КонецЕсли;
КонецПроцедуры

&НаКлиенте
Процедура AMS_ЗаполнитьТоварыДляПеремещения(Объект)
	Данные = AMS_ПолучитьОстатокНеПереданныхТоваровИзПроизводства(Объект._Основание);
	
	Если Данные = Неопределено Тогда
		Возврат;
	КонецЕсли;

	Товары = Объект.Товары; Товары.Очистить();
	Серии  = Объект.Серии; Серии.Очистить();
	...
	КоличествоПалет = Данные.КоличествоПалетМоно + ?(Данные.КоличествоПалетСборных > 1, 1, 0);

	Если КоличествоПалет > 0 Тогда
		НовСтр = Товары.Добавить();
		
		НовСтр.Номенклатура       = Данные.Палета;
		НовСтр.КоличествоУпаковок = КоличествоПалет;
		НовСтр.Количество         = КоличествоПалет;
	КонецЕсли;
КонецПроцедуры

&НаКлиенте
Процедура AMS_ЗаполнитьФлаконыДляПеремещения(Объект)
	Данные = AMS_ПолучитьОстатокНеПереданныхФлаконовИзПроизводства(Объект._Основание);
...
&НаСервере
Функция AMS_ПолучитьОстатокНеПереданныхТоваровИзПроизводства(Основание)
	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("Основание", Основание);
	Запрос.УстановитьПараметр("Дата", ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Основание, "Дата"));
	Запрос.УстановитьПараметр("ПеремещениеТоваров", Объект.Ссылка);
	Запрос.УстановитьПараметр("ВидыНоменклатурыПродукция", AMS_ВызовСервера.ВидыНоменклатурыПродукции());

	Запрос.Текст =
	"ВЫБРАТЬ
	|	т.Номенклатура КАК Номенклатура,
	|	т.Количество КАК Количество,
	|	т.Номенклатура.ВидНоменклатуры.ИспользоватьСрокГодностиСерии КАК ИспользоватьСрокГодности,
	|	НАЧАЛОПЕРИОДА(&Дата, ДЕНЬ) КАК ДатаПроизводства,
	|	НАЧАЛОПЕРИОДА(ДОБАВИТЬКДАТЕ(&Дата, МЕСЯЦ, Номенклатура.СрокГодности), ДЕНЬ) КАК ГоденДо
	|ПОМЕСТИТЬ Сделано
	|ИЗ
	|	Документ.ПрочееОприходованиеТоваров.Товары КАК т
	|ГДЕ
	|	т.Ссылка._Основание = &Основание
	|	И т.Ссылка._ОперацияПроизводства = ЗНАЧЕНИЕ(Перечисление.AMS_ОперацииПроизводства.ПроизводствоПродукции)
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|ГДЕ
	|	т.Ссылка._Основание = &Основание
	|	И т.Ссылка._ОперацияПроизводства = ЗНАЧЕНИЕ(Перечисление.AMS_ОперацииПроизводства.ПеремещениеПроизведеннойПродукции)
	|	И НЕ т.Ссылка.ПометкаУдаления
	|	И т.Номенклатура.ВидНоменклатуры В (&ВидыНоменклатурыПродукция)
	|	И НЕ т.Ссылка = &ПеремещениеТоваров
	|СГРУППИРОВАТЬ ПО
	|	т.Номенклатура
	|;
	|
	...
	|	Сделано.ИспользоватьСрокГодности КАК ИспользоватьСрокГодности,
	|	Сделано.ДатаПроизводства КАК ДатаПроизводства,
	|	Сделано.ГоденДо КАК ГоденДо,
	...
	|	вт.Номенклатура._Упаковка КАК Упаковка,
	|	вт.Количество КАК Количество,
	|	вт.Количество / вт.Номенклатура.КоэффициентЕдиницыДляОтчетов КАК КоличествоУпаковок
	|ИЗ
	|	вт КАК вт
	|ГДЕ
	|	вт.Количество > 0
	...
	КолПалет = AMS_Сервер.ПолучитьВесОбъемТоваров(Товары);
	
	МассивТовары = Новый Массив();
	
	Для Каждого Стр Из Товары Цикл
		МассивТовары.Добавить(ОбщегоНазначения.СтрокаТаблицыЗначенийВСтруктуру(Стр));
	КонецЦикла;

	Данные = Новый Структура();
	Данные.Вставить("Товары", МассивТовары);
	Данные.Вставить("КоличествоПалетМоно", КолПалет.КоличествоПалетМоно);
	Данные.Вставить("КоличествоПалетСборных", КолПалет.КоличествоПалетСборных);
	Данные.Вставить("Палета", Справочники.Номенклатура.НайтиПоКоду("01-0002891"));
	
	Возврат Данные;
КонецФункции

&НаСервере
Функция AMS_ПолучитьОстатокНеПереданныхФлаконовИзПроизводства(Основание)
	Если Объект.СкладПолучатель = AMS_ВызовСервера.Цех1() Тогда
		Сегмент = AMS_ВызовСервера.СегментыНоменклатурыУпаковкаЦеха1();	
	
	ИначеЕсли Объект.СкладПолучатель = AMS_ВызовСервера.Цех2() Тогда
		Сегмент = AMS_ВызовСервера.СегментыНоменклатурыУпаковкаЦеха2();	
	
	ИначеЕсли Объект.СкладПолучатель = AMS_ВызовСервера.Цех3() Тогда
	...
	|ИЗ
	|	Документ.ПрочееОприходованиеТоваров.Товары КАК т
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.НоменклатураСегмента КАК НомСегм
	|		ПО т.Номенклатура = НомСегм.Номенклатура
	|ГДЕ
	|	т.Ссылка._Основание = &Основание
	|	И НомСегм.Сегмент = &Сегмент
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	Номенклатура
	|;
	...
	|	И т.Ссылка._ОперацияПроизводства = ЗНАЧЕНИЕ(Перечисление.AMS_ОперацииПроизводства.ПеремещениеПроизведеннойПродукции)
	|	И НЕ т.Ссылка.ПометкаУдаления
	|	И НЕ т.Ссылка.Ссылка = &ПеремещениеТоваров
	|СГРУППИРОВАТЬ ПО
	|	т.Номенклатура
	|
	|ИНДЕКСИРОВАТЬ ПО
	...
	Данные = Новый Структура();
	Данные.Вставить("Товары", МассивТовары);
	Данные.Вставить("КоличествоПалетМоно", 1);
	Данные.Вставить("КоличествоПалетСборных", 1);
	Данные.Вставить("Палета", Справочники.Номенклатура.НайтиПоКоду("01-0002891"));
...
Процедура AMS_УправлениеЭлементамиФормы()
	AMS_СобытияФорм.УправлениеФормаДокументаПеремещениеТоваров(ЭтотОбъект);
КонецПроцедуры	
