...
Процедура ЗаполнитьПоОснованиюНаСервере()
	...
	//!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~2
	ВремОбъект.Товары.Сортировать("Номенклатура");
КонецПроцедуры
...
&НаКлиенте
Процедура Подключаемый_амсКоманда_ЗаполнитьЦены(Команда)
	СтруктураДействий = Новый Структура;
	СтруктураДействий.Вставить("ПересчитатьСумму", "Количество");

	Табл = амсПолучитьЦеныТоваров();
	
	Товары = Объект.Товары;
	Для каждого Стр Из Табл Цикл
		Если Стр.Цена > 0  Тогда 
			Строки = Товары.НайтиСтроки(Новый Структура("Номенклатура", Стр.Номенклатура));
			...
			Если Строки.Количество() > 0  Тогда 
				...
				ПакетнаяОбработкаТабличнойЧастиКлиент.ОбработатьСтрокуТЧ(ТекСтр, СтруктураДействий, КэшированныеЗначения);
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;
КонецПроцедуры
...
&НаКлиенте
Процедура Подключаемый_амсКоманда_ЗаполнитьМатериалы(Команда)
	амсЗаполнитьТовары(амсПолучитьМатериалы());
КонецПроцедуры

&НаКлиенте
Процедура Подключаемый_амсКоманда_ЗаполнитьФабрикаты(Команда)
	амсЗаполнитьТовары(амсПолучитьФабрикаты());
КонецПроцедуры

&НаКлиенте
Процедура амсЗаполнитьТовары(Таб)
...
&НаСервере
Функция амсПолучитьЦеныТоваров()
	МВТ = Новый МенеджерВременныхТаблиц;

	Парам = Новый Структура;
	Парам.Вставить("Дата", Объект.Дата);
	Парам.Вставить("Номенклатура", Объект.Товары.Выгрузить(, "Номенклатура").ВыгрузитьКолонку("Номенклатура"));
...
Функция амсПолучитьМатериалы()
	...
	|	И Номенклатура.ВидНоменклатуры = &ВидНоменклатурыМатериал) КАК т
	|ГДЕ
	|	т.ВНаличииОстаток < 0
	|
	|УПОРЯДОЧИТЬ ПО
