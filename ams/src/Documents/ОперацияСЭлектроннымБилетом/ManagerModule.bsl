...
//  Массив из Строка
//
Функция КлючевыеРеквизитыЭлектронногоБилета(ДляСинхронизации = Истина) Экспорт
...
Процедура АктуализироватьОперацииИСостояниеЭлектронногоБилета(СсылкаНаДокумент) Экспорт
	...
	КлючевыеРеквизиты = КлючевыеРеквизитыЭлектронногоБилета();
	КлючевыеРеквизитыУдаленнойОперации = КлючевыеРеквизитыЭлектронногоБилета(Ложь);
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	Запрос.Текст = 
		...
		|	ФайлыБронирований.Наименование КАК Наименование,
		|	ФайлыБронирований.Автор КАК Автор,
		|	ФайлыБронирований.ДатаЗаема КАК ДатаЗаема,
		|	ФайлыБронирований.ДатаМодификацииУниверсальная КАК ДатаМодификацииУниверсальная,
		...
		|	ФайлыБронирований.Расширение КАК Расширение,
				...
				Если ВыборкаДокументов.ТребуетсяСозданиеНовогоБронирования Тогда
					...
					Если ВыборкаФайлов <> Неопределено Тогда
						...
						Пока ВыборкаФайлов.Следующий() Цикл
							...
							НовыйФайлБронирования = Справочники.БронированиеSmartwayПрисоединенныеФайлы.СоздатьЭлемент();
							НовыйФайлБронированияСсылка = Справочники.БронированиеSmartwayПрисоединенныеФайлы.ПолучитьСсылку();
							НовыйФайлБронирования.УстановитьСсылкуНового(НовыйФайлБронированияСсылка);
							
							ЗаполнитьЗначенияСвойств(НовыйФайлБронирования, ВыборкаФайлов);
							НовыйФайлБронирования.ВладелецФайла = НовыйДокументБронирования.Ссылка;
							ДвоичныеДанные = РаботаСФайлами.ДвоичныеДанныеФайла(ВыборкаФайлов.ПрисоединенныйФайл);
							
							Если ВыборкаФайлов.ТипХраненияФайла = Перечисления.ТипыХраненияФайлов.ВИнформационнойБазе Тогда
								НовыйФайлБронирования.ТипХраненияФайла = ВыборкаФайлов.ТипХраненияФайла;
								РаботаСФайламиСлужебный.ЗаписатьФайлВИнформационнуюБазу(НовыйФайлБронированияСсылка, ДвоичныеДанные);
							Иначе
								НовыйФайлБронирования.Том = Неопределено;
								НовыйФайлБронирования.ПутьКФайлу = Неопределено;
								НовыйФайлБронирования.ТипХраненияФайла = Неопределено;
								РаботаСФайламиВТомахСлужебный.ДобавитьФайл(НовыйФайлБронирования, ДвоичныеДанные);
							КонецЕсли;
							НовыйФайлБронирования.Записать();
