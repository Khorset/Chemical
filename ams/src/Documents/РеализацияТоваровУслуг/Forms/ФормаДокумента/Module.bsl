...
Процедура ПриОткрытии(Отказ)
	...
	//!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~2
	Если AMS_ВызовСервера.РолиДоступны("_КладовщикСклада") 
		И Не (AMS_ВызовСервера.ЭтоKhors() Или AMS_ВызовСервера.ЭтоМеренков()) Тогда
		Отказ = Истина;
		Возврат;
	КонецЕсли;
	...
	Если Параметры.Ключ.Пустая() Тогда
		Если СтрНайти(Объект.Комментарий, "Контур-EDI") > 0 Тогда
			AMS_ОбработатьДокументКонтурEDI(Отказ);
		КонецЕсли;
	КонецЕсли;
	
	AMS_УправлениеЭлементамиФормы();
КонецПроцедуры
...
Процедура ПослеЗаписиНаСервере(ТекущийОбъект, ПараметрыЗаписи)
	...
	//!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~4
	AMS_ЗаписатьИсториюРеализации(ТекущийОбъект, ПараметрыЗаписи);
КонецПроцедуры
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	Если ИмяСобытия = "Запись_ПравилаПродаж" И Источник = Объект.AMS_ПравилоПродажи Тогда
		AMS_УстановитьПравилаПродажи();
	КонецЕсли;
	//!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~5
...
Процедура ПартнерПриИзменении(Элемент)
	...
	//!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~7
	ПравилоПродажи = AMS_ВызовСервера.ПолучитьПравилоПродажи(Объект.Дата, Объект.Партнер);
	Если ЗначениеЗаполнено(ПравилоПродажи) Тогда
		Объект.AMS_ПравилоПродажи = ПравилоПродажи; 
		AMS_УстановитьПравилаПродажи();
	КонецЕсли;
	AMS_УправлениеЭлементамиФормы();
КонецПроцедуры
...
Процедура ДатаПриИзмененииСервер(ПересчитатьЦены, ЦеныРассчитаны, ПрименитьОплатуБонуснымиБаллами = Ложь)
	...
	Если ПересчитатьЦены Тогда
		ПроверитьНаличиеСкидокОплатБонуснымиБалламиЛокализация();
	КонецЕсли;
...
Процедура СпособМестоДоставкиПеревозчикПриИзменении(Элемент)
	...
	//!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~14
	Если Объект.СпособДоставки = ОбщегоНазначенияКлиент.ПредопределенныйЭлемент("Перечисление.СпособыДоставки.СиламиПеревозчикаПоАдресу") Тогда
		Объект.ДоверенностьНомер = ПрефиксацияОбъектовКлиентСервер.НомерНаПечать(Объект.Номер, Ложь, Истина);
		Объект.ДоверенностьДата  = Объект.Дата;
	Иначе
		Объект.ДоверенностьНомер = ""; 
		Объект.ДоверенностьДата  = Дата(1,1,1);
	КонецЕсли;
	
	Если Объект.СпособДоставки = ОбщегоНазначенияКлиент.ПредопределенныйЭлемент("Перечисление.СпособыДоставки.ДоКлиента")
		Или Объект.СпособДоставки = ОбщегоНазначенияКлиент.ПредопределенныйЭлемент("Перечисление.СпособыДоставки.КПолучателюОпределяетСлужбаДоставки") Тогда
		Объект.ЗонаДоставки = AMS_ВызовСервера.ПолучитьЗонаДоставки(Объект.Партнер, Объект.АдресДоставки);
		
	ИначеЕсли Объект.СпособДоставки = ОбщегоНазначенияКлиент.ПредопределенныйЭлемент("Перечисление.СпособыДоставки.СиламиПеревозчикаПоАдресу") Тогда
		Объект.ЗонаДоставки = AMS_ВызовСервера.ПолучитьЗонаДоставки(Объект.ПеревозчикПартнер, Объект.АдресДоставкиПеревозчика);
...
&НаСервере
Процедура ПроверитьНаличиеСкидокОплатБонуснымиБалламиЛокализация()
	...
	РозничныеПродажиЛокализация.ПроверитьНаличиеСкидокОплатБонуснымиБаллами(ЭтотОбъект, "Товары", Истина, Истина);
	...
	ОписаниеЛога.Вставить("Метод", "ПроверитьНаличиеСкидокОплатБонуснымиБалламиЛокализация");
...
#Область AMS_ПодключаемыеКомандыПечати

&НаКлиенте
Процедура Подключаемый_AMS_КомандаПечатьЗаказКлиента(Команда)
	AMS_Клиент.ВыполнитьКомандуПечати(Объект.Ссылка, "СчетНаОплату", Новый Структура("Заголовок", "ЗаказКлиента"));
КонецПроцедуры

&НаКлиенте
Процедура Подключаемый_AMS_Команда_ПечатьСчетНаОплату(Команда)
	AMS_Клиент.ВыполнитьКомандуПечати(Объект.Ссылка, "СчетНаОплату", Новый Структура("Заголовок", "Счет"));
КонецПроцедуры

&НаКлиенте
Процедура Подключаемый_амсКоманда_ПечатьУПД(Команда)
	AMS_ВыполнитьКомандуПечати(Объект.Ссылка,"УПД")
КонецПроцедуры

&НаКлиенте
Процедура Подключаемый_амсКоманда_ПечатьСкладскаяНакладная(Команда)
	AMS_ВыполнитьКомандуПечати(Объект.Ссылка,"СН");
КонецПроцедуры

&НаКлиенте
Процедура Подключаемый_амсКоманда_ПечатьУпаковочныйЛист(Команда)
	AMS_ВыполнитьКомандуПечати(Объект.Ссылка,"УпаковочныйЛист");
КонецПроцедуры

&НаКлиенте
Процедура Подключаемый_амсКоманда_ПечатьДоверенность(Команда)
	AMS_ВыполнитьКомандуПечати(Объект.Ссылка,"Доверенность");
КонецПроцедуры

&НаКлиенте
Процедура Подключаемый_амсКоманда_ПечатьТН(Команда)
	Парам = Новый Структура("ТранспортноеСредство, Распоряжение, Образец", Неопределено, Объект.Ссылка, Истина);
	
	AMS_ВыполнитьКомандуПечати(Объект.Ссылка, "ТН", Парам);
КонецПроцедуры

&НаКлиенте
Процедура Подключаемый_амсКоманда_ПечатьТТН(Команда)
	AMS_ВыполнитьКомандуПечати(Объект.Ссылка,"ТТН");
КонецПроцедуры

&НаКлиенте
Процедура Подключаемый_амсКоманда_ПечатьТОРГ12(Команда)
	AMS_ВыполнитьКомандуПечати(Объект.Ссылка,"ТОРГ12",Новый Структура("ВыводитьУслуги", Ложь));
КонецПроцедуры

&НаКлиенте
Процедура Подключаемый_амсКоманда_ПечатьСчетФактура(Команда)
	AMS_ВыполнитьКомандуПечати(Объект.Ссылка,"СчетФактура",Новый Структура("ПечатьВВалюте", Ложь));
КонецПроцедуры

&НаКлиенте
Процедура Подключаемый_амсКоманда_ПечатьСпецификация(Команда)
	AMS_ВыполнитьКомандуПечати(Объект.Ссылка,"Спецификация");
КонецПроцедуры

&НаКлиенте
Процедура Подключаемый_амсКоманда_ПечатьСпецификацияЭкспорт(Команда)
	AMS_ВыполнитьКомандуПечати(Объект.Ссылка,"СпецификацияЭкспорт");
КонецПроцедуры

&НаКлиенте
Процедура Подключаемый_амсКоманда_ПечатьАктаОРасхождениях(Команда)
	AMS_ВыполнитьКомандуПечати(Объект.Ссылка,"АктОРасхождениях");
КонецПроцедуры

&НаКлиенте
Процедура Подключаемый_амсКоманда_ПечатьПалетнаяЭтикетка(Команда)
	AMS_ВыполнитьКомандуПечати(Объект.Ссылка,"ПалетнаяЭтикетка");
КонецПроцедуры

&НаКлиенте
Процедура Подключаемый_амсКоманда_ПечатьЗаявкаПеревозчику(Команда)
	Если Объект.СпособДоставки = ОбщегоНазначенияКлиент.ПредопределенныйЭлемент("Перечисление.СпособыДоставки.СиламиПеревозчикаПоАдресу") Тогда
		AMS_ВыполнитьКомандуПечати(Объект.Ссылка,"ЗаявкаПеревозчику");
	КонецЕсли;
КонецПроцедуры

&НаКлиенте
Процедура Подключаемый_амс_ПечатьСертификатыНоменклатурыРеестр(Команда)
	AMS_ВыполнитьКомандуПечати(Объект.Ссылка,"СертификатыНоменклатурыРеестр");
КонецПроцедуры

&НаКлиенте
Процедура Подключаемый_амс_ПечатьСертификатыНоменклатурыБезДублей(Команда)
	AMS_ВыполнитьКомандуПечати(Объект.Ссылка,"СертификатыНоменклатурыИзображенияИзДокументовБезДублей");
КонецПроцедуры

&НаКлиенте
Процедура Подключаемый_амс_ПечатьНакладнаяНаПалеты(Команда)
	AMS_ВыполнитьКомандуПечати(Объект.Ссылка,"НакладнаяНаПалеты");
КонецПроцедуры

#КонецОбласти

#Область AMS_ПодключаемыеКоманды

&НаКлиенте
Процедура Подключаемый_AMS_КомандаПодборТоваров(Команда)
	Отказ = Ложь;

	AMS_ВызовСервера.AMS_ПроверитьВозможностьПодбора(Объект, Отказ);
	
	Если Отказ Тогда 
		Возврат;
	КонецЕсли;
	
	Парам = Новый Структура("АдресДанных", AMS_ВызовСервера.ПоместитьВоВременноеХранилищеТовары(Объект));
		
	ОткрытьФорму("Обработка.AMS_ПодборТоваров.Форма",Парам,,,,,
		Новый ОписаниеОповещения("AMS_ПодборТоваровЗавершение", ЭтотОбъект),
		РежимОткрытияОкнаФормы.БлокироватьВесьИнтерфейс);
КонецПроцедуры

&НаКлиенте
Процедура Подключаемый_AMS_Команда_ЗагрузитьИзДокументаРеализации(Команда)
	Отказ = Ложь;
...
&НаКлиенте
Процедура Подключаемый_AMS_Команда_ОтчетОДоступностиТовара(Команда)
	Если Модифицированность Тогда
		ПоказатьПредупреждение(Неопределено, "Документ изменён." + Символы.ПС + "Необходимо записать изменения.");
		Возврат;
	КонецЕсли;
	...
	Парам.Вставить("КлючНазначенияИспользования"    ,"ДоступностьТоваров");
	...
	ОткрытьФорму("Отчет.AMS_ДоступностьТоваров.Форма",Парам,ЭтотОбъект);
КонецПроцедуры

#КонецОбласти

#Область AMS_ПодключаемыеСобытия

&НаКлиенте
Процедура Подключаемый_AMS_ПриИзменении_СтатусРеализации(Элемент)
	Если НЕ (AMS_ВызовСервера.РолиДоступны("_Директор,_ГлавныйБухгалтер,_Бухгалтер") или AMS_ВызовСервера.ЭтоKhors()) 
		И НЕ (Объект.AMS_СтатусРеализации = ОбщегоНазначенияКлиент.ПредопределенныйЭлемент("Перечисление.AMS_СтатусыРеализаций.Заказ") 
			Или Объект.AMS_СтатусРеализации = ОбщегоНазначенияКлиент.ПредопределенныйЭлемент("Перечисление.AMS_СтатусыРеализаций.КОтгрузке")
			Или Объект.AMS_СтатусРеализации = ОбщегоНазначенияКлиент.ПредопределенныйЭлемент("Перечисление.AMS_СтатусыРеализаций.Стикерование")) Тогда
			    	
	    Текст = "Статус """ + Объект.AMS_СтатусРеализации + """ может установить бухгалтер.";
		ОбщегоНазначенияКлиент.СообщитьПользователю(Текст,,"AMS_СтатусРеализации","Объект");
		
		Объект.AMS_СтатусРеализации = ЭтотОбъект.AMS_мСтатусРеализации;
		
		Модифицированность = Ложь;
	
	ИначеЕсли Объект.AMS_СтатусРеализации = ОбщегоНазначенияКлиент.ПредопределенныйЭлемент("Перечисление.AMS_СтатусыРеализаций.Стикерование")
		И Не AMS_ВызовСервера.ПолучитьНеобходимостьСтикерованияТовара(Объект.Партнер) Тогда
	    
	    Текст = "Для клиента """ + Объект.Партнер + """ товар не стикеруется.";
		ОбщегоНазначенияКлиент.СообщитьПользователю(Текст,,"AMS_СтатусРеализации","Объект");
	...
	Иначе
		...
		AMS_мСтатусРеализации = Объект.AMS_СтатусРеализации;
		AMS_УправлениеЭлементамиФормы();
	КонецЕсли;
КонецПроцедуры

&НаКлиенте
Процедура Подключаемый_AMS_ПриИзменении_УтвержденаОтгрузка(Элемент)
	Если Не Объект.AMS_УтвержденаОтгрузка Или AMS_ВызовСервера.ДоступнаРоль("_ГлавныйБухгалтер") Тогда
		Возврат;
	КонецЕсли;
	...
	БезОграничений = AMS_ВызовСервера.ЕстьПравоАМС(ПредопределенноеЗначение("Перечисление.AMS_Права.ПроведениеРеализацииБезОграничений")) Или AMS_ВызовСервера.ЭтоKhors();
	
	// Разрешить отклонение от установленных цен
	Если Не БезОграничений И Объект.AMS_ЦенаИзменена Тогда
		Текст = "Найдены цены не соответствующие условиям продажи. Отгрузку может разрешить старший менеджер.";
		ОбщегоНазначенияКлиент.СообщитьПользователю(Текст, , , , Отказ);
	КонецЕсли;
	
	// Проверка возможности склада
	ВозможностиСклада  = AMS_ВызовСервера.ПолучитьВозможностиСклада(Объект.Дата);
	КоличествоУпаковок = Объект.Товары.Итог("КоличествоУпаковок");
	КоличествоРезерв   = AMS_ПолучитьРезерв(Объект.Ссылка, Объект.Дата);

	Если КоличествоРезерв + КоличествоУпаковок > ВозможностиСклада Тогда
		Текст = "Продажи " + Формат(Объект.Дата, "ДЛФ=Д") + " превышают возможности склада по отгрузке товаров."
			+ Символы.ПС + "Резерв " + КоличествоРезерв + " упак." + Символы.ПС + "В документе " + КоличествоУпаковок + " упак."
			+ Символы.ПС + "Возможности склада " + ВозможностиСклада + " упак.";
		ОбщегоНазначенияКлиент.СообщитьПользователю(Текст, , , , Отказ);
	КонецЕсли;
	
	// Проверить "Основание"
	Если ЗначениеЗаполнено(Объект.Договор) И ПустаяСтрока(Объект.Основание) Тогда
		Текст = "Не заполнено поле ""Основание""";
		ОбщегоНазначенияКлиент.СообщитьПользователю(Текст, , , , Отказ);
	КонецЕсли;
	
	// Проверить при Золушка и АМС Трейд
	Если Не Объект.Организация = AMS_ВызовСервера.ОрганизацияСобственникПродукции() Тогда
		Если Объект.Организация = AMS_ВызовСервера.ОрганизацияТД() Тогда
			СкладЗолушки = AMS_ВызовСервера.СкладЗолушки();
			Если Не Объект.Склад = СкладЗолушки Тогда
				Текст = "Отгрузка должна быть выполнена со склада ""%1""";
				Текст = СтрШаблон(Текст, СкладЗолушки);
				ОбщегоНазначенияКлиент.СообщитьПользователю(Текст, Объект.Ссылка, "Объект.Склад", , Отказ);
			КонецЕсли;
		КонецЕсли;
		
		Если Объект.Организация = AMS_ВызовСервера.ОрганизацияТрейд() Тогда
			СкладТрейд = AMS_ВызовСервера.СкладТрейд();
			Если Не Объект.Склад = СкладТрейд Тогда
				Текст = "Отгрузка должна быть выполнена со склада ""%1""";
				Текст = СтрШаблон(Текст, СкладТрейд);
				ОбщегоНазначенияКлиент.СообщитьПользователю(Текст, Объект.Ссылка, "Объект.Склад", , Отказ);
			КонецЕсли;
		КонецЕсли;

		AMS_ПроверитьДокументыПередачиТоваров(Отказ);
	КонецЕсли;
	
	// Проверить дубли строк
	AMS_ПроверитьДублиНоменклатуры(Отказ);
	...
	МассивТоваров = AMS_ВызовСервера.ПолучитьТоварыЗапрещенныеКПродажиЧестныйЗнакМасло(Объект.Контрагент, Объект.Дата, МассивТоваров);
	Для Каждого Товар Из МассивТоваров Цикл
		Текст = "Номенклатура ""%1"" запрещена к продажи контрагенту ""%2""";
		Текст = СтрШаблон(Текст, Товар, Объект.Контрагент);
		ОбщегоНазначенияКлиент.СообщитьПользователю(Текст,,,, Отказ);
	КонецЦикла;
	
	МассивТоваров = AMS_ВызовСервера.ПолучитьТоварыЗапрещенныеКПродажиЧестныйЗнакКосметика(Объект.Контрагент, Объект.Дата, МассивТоваров);
	Для Каждого Товар Из МассивТоваров Цикл
		Текст = "Номенклатура ""%1"" запрещена к продажи контрагенту ""%2""";
		Текст = СтрШаблон(Текст, Товар, Объект.Контрагент);
		ОбщегоНазначенияКлиент.СообщитьПользователю(Текст,,,, Отказ);
	...
	// Проверить оплату
	Если Не Отказ Тогда
		Если AMS_ЭтоПредоплата() Тогда
			ДокументыПредоплаты = AMS_ПолучитьДокументыПредоплаты(Объект.Ссылка);
			
			Если ДокументыПредоплаты.Платежка = Неопределено Тогда
				ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru='Нет предоплаты.'"),,,,Отказ);
			КонецЕсли;

			Если Не Объект.НалогообложениеНДС = ПредопределенноеЗначение("Перечисление.ТипыНалогообложенияНДС.ПродажаНаЭкспорт")
				И ДокументыПредоплаты.СчетФактураНаАванс = Неопределено Тогда
				Текст = НСтр("ru='Не оформлен счёт-фактура на аванс. Обратитесь к бухгалтеру.'");
				ОбщегоНазначенияКлиент.СообщитьПользователю(Текст,,,,Отказ);
			КонецЕсли;

			ДолгПоДокументу = AMS_ДолгПоДокументу();
			
			Если ДолгПоДокументу > 0 Тогда
				Текст = НСтр("ru='Долг по предоплате составляет %1 руб. Отгрузку может разрешить главный бухгалтер.'");
				Текст = СтрШаблон(Текст, ДолгПоДокументу);
				ОбщегоНазначенияКлиент.СообщитьПользователю(Текст,,,,Отказ);
			КонецЕсли;
			
		Иначе 
			Стр = AMS_ПолучитьДолги();

			// Это продажа в кредит - проверить превышение лимита
			Лимит = ОбщегоНазначенияУТВызовСервера.ЗначениеРеквизитаОбъекта(Объект.Партнер, "AMS_Лимит");
			Если Лимит > 0 Тогда
				Превышение  = Стр.ДолгКлиента - Лимит;
				Если Превышение > 0 Тогда
					Текст = НСтр("ru='Превышение лимита задолженности составляет %1 руб. Отгрузку может разрешить главный бухгалтер.'");
					Текст = СтрШаблон(Текст, Превышение);
					ОбщегоНазначенияКлиент.СообщитьПользователю(Текст,,,,Отказ);
				КонецЕсли;
			КонецЕсли;
			
			// Это продажа в кредит - проверить превышение сумму фиксированного договора
			СуммаДоговора = ОбщегоНазначенияУТВызовСервера.ЗначениеРеквизитаОбъекта(Объект.Договор, "Сумма");
			Если Не СуммаДоговора = Неопределено И СуммаДоговора > 0  Тогда
				СуммаИсполнено = AMS_ПолучитьСуммуВыполненияДоговора(Объект.Договор, Объект.Ссылка);
				Превышение     = СуммаИсполнено - СуммаДоговора;
				Если Превышение > 0 Тогда
					Текст = "Сумма контракта %1 руб., исполнено %2 руб., превышение составляет %3 руб.  Отгрузку может разрешить главный бухгалтер.";
					Текст = СтрШаблон(Текст, СуммаДоговора, СуммаИсполнено, Превышение);
					ОбщегоНазначенияКлиент.СообщитьПользователю(Текст,,,,Отказ);
				КонецЕсли;
			КонецЕсли;
			
			// Это продажа в кредит - проверить просроченный долг
			Если Стр.МассивСтрок.Количество() > 0 Тогда
				Парам = Новый Структура("МассивСтрок, ДолгКлиентаПросрочен, ПроведениеБезОграничений", Стр.МассивСтрок, Стр.ДолгКлиентаПросрочен, БезОграничений);
						
				ОткрытьФорму("Документ.РеализацияТоваровУслуг.Форма.AMS_ФормаПросроченныеДолги",Парам,,,,,
						Новый ОписаниеОповещения("AMS_РазрешитьОтгрузкуЗавершение",ЭтотОбъект),
						РежимОткрытияОкнаФормы.БлокироватьВесьИнтерфейс);
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
	...
	Если Отказ Тогда
		Объект.AMS_УтвержденаОтгрузка = Ложь;
		Модифицированность = Ложь;
	Иначе
		Объект.AMS_КоличествоПалетПлан = 0;
		Объект.AMS_ВесПлан             = 0;
	КонецЕсли;
КонецПроцедуры

&НаКлиенте
Процедура Подключаемый_AMS_ПриИзменении_ПравилоПродажи(Элемент)
	AMS_УстановитьПравилаПродажи();
КонецПроцедуры

&НаКлиенте
Процедура Подключаемый_амсПриИзменении_КоличествоПалетПлан(Элемент)
	Если Объект.AMS_КоличествоПалетПлан < Объект.AMS_КоличествоПалет Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю("Указанное количество должно быть равно или больше значения в поле ""Количество палет""");
		Объект.AMS_КоличествоПалетПлан = 0;
	КонецЕсли;
КонецПроцедуры

&НаКлиенте
Процедура Подключаемый_амсПриИзменении_ВесПлан(Элемент)
	Если Объект.AMS_ВесПлан < Объект.AMS_Вес Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю("Указанное количество должно быть равно или больше значения в поле ""Вес""");
		Объект.AMS_ВесПлан = 0;
	КонецЕсли;
КонецПроцедуры

&НаКлиенте
Процедура Подключаемый_AMS_ПриИзменении_Основание(Элемент)
	Если Объект.Основание = Основание Тогда
		ОснованиеИзменено = Ложь;
	Иначе
...
&НаКлиенте
Процедура Подключаемый_AMS_ПриИзменении_ПартнерДопИнфо(Элемент)
	Если ЗначениеЗаполнено(Объект.Партнер) Тогда 
		//@skip-check module-self-reference
		AMS_ВызовСервера.ИзменитьПартнерДопИнфо(Объект.Партнер, ЭтотОбъект.AMS_ПартнерДопИнфо);
	КонецЕсли;
КонецПроцедуры

#КонецОбласти

#Область СлужебныеПроцедурыИФункцииАМС

&НаКлиенте
Процедура AMS_ОткрытьФормуДобавленияТовараЗавершение(Результат,ДополнительныеПераметры) Экспорт
	Если Не Результат = Неопределено Тогда
		AMS_ПеренестиДанные(Результат);
	КонецЕсли;
КонецПроцедуры

&НаКлиенте
Процедура AMS_РазрешитьОтгрузкуЗавершение(Результат,ДополнительныеПераметры) Экспорт
	Если Результат = Истина Тогда
		Объект.AMS_УтвержденаОтгрузка  = Истина;
		Объект.AMS_КоличествоПалетПлан = 0;
		Объект.AMS_ВесПлан             = 0;
...
&НаСервереБезКонтекста
Функция AMS_ПолучитьДокументыПредоплаты(Ссылка)
	Запрос = Новый Запрос;
	...
	|	вт.Платежка КАК Платежка,
	|	СчетФактураАванс.Ссылка КАК СчетФактураНаАванс
	|ИЗ
	|	вт КАК вт
	...
	ДокументыПредоплаты = Новый Структура("Платежка, СчетФактураНаАванс", Неопределено, Неопределено);
	
	Если Табл.Количество() > 0 Тогда
		Платежка = Табл[0].Платежка;
		Если ЗначениеЗаполнено(Платежка) Тогда
			ДокументыПредоплаты.Платежка = Платежка;
		КонецЕсли;

		СчетФактураНаАванс = Табл[0].СчетФактураНаАванс;
		Если ЗначениеЗаполнено(СчетФактураНаАванс) Тогда
			ДокументыПредоплаты.СчетФактураНаАванс = СчетФактураНаАванс;
		КонецЕсли;
	КонецЕсли;
	
	Возврат ДокументыПредоплаты;
КонецФункции

&НаСервере
Функция AMS_ПолучитьДолги()
	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("Организация", Объект.Организация);
	Запрос.УстановитьПараметр("Партнер", Объект.Партнер);
	...
	|	т.АналитикаУчетаПоПартнерам.Партнер КАК Клиент,
	...
	|	РАЗНОСТЬДАТ(т.ДатаПлановогоПогашения, &ТекДата, ДЕНЬ) КАК Просрочено
	|ПОМЕСТИТЬ Расчеты
	|ИЗ
	|	РегистрНакопления.РасчетыСКлиентамиПоСрокам.Остатки КАК т
	|ГДЕ
	|	т.АналитикаУчетаПоПартнерам.Организация = &Организация
	|	И т.АналитикаУчетаПоПартнерам.Партнер = &Партнер
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|	Расчеты.ДолгКлиентаПросрочен КАК ДолгКлиентаПросрочен,
	|	Расчеты.Просрочено КАК Просрочено
	|ИЗ
	|	Расчеты КАК Расчеты
	|
	|УПОРЯДОЧИТЬ ПО
	...
	Предоплата  = Табл.Итог("Предоплата");
	ДолгКлиента = Табл.Итог("ДолгКлиента");
	ДолгКлиентаПросрочен = Табл.Итог("ДолгКлиентаПросрочен");
	
	МассивСтрок = Новый Массив();
	
	Для Каждого Стр Из Табл Цикл
		Если Стр.ДолгКлиентаПросрочен Тогда
			МассивСтрок.Добавить(ОбщегоНазначения.СтрокаТаблицыЗначенийВСтруктуру(Стр));
		КонецЕсли;
	КонецЦикла;
	
	//@skip-check structure-consructor-too-many-keys
	Возврат Новый Структура("Предоплата, ДолгКлиента, ДолгКлиентаПросрочен, МассивСтрок", Предоплата, ДолгКлиента, ДолгКлиентаПросрочен, МассивСтрок);
КонецФункции

&НаСервере
Функция AMS_ЭтоПредоплата()
	Если Объект.ЭтапыГрафикаОплаты.Количество() > 0 Тогда
		Сдвиг = Объект.ЭтапыГрафикаОплаты[0].Сдвиг;
		
	Иначе
		Запрос = Новый Запрос;
		Запрос.УстановитьПараметр("ГрафикОплаты", ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Объект.Соглашение, "ГрафикОплаты"));
		Запрос.Текст =
		"ВЫБРАТЬ ПЕРВЫЕ 1
		|	т.Сдвиг КАК Сдвиг
		|ИЗ
		...
		Если Рез.Пустой() Тогда
			Сдвиг = Неопределено;
		Иначе
			Выб = Рез.Выбрать(); Выб.Следующий(); Сдвиг = Выб.Сдвиг;
		КонецЕсли;
	КонецЕсли;	
...
&НаСервере
Функция AMS_ДолгПоДокументу()
	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("ОбъектРасчетов", Объект.Ссылка);
	Запрос.Текст =
	"ВЫБРАТЬ
	...
	|	РегистрНакопления.РасчетыСКлиентамиПоСрокам.Остатки КАК т
	|ГДЕ
	|	т.РасчетныйДокумент = &ОбъектРасчетов";
	...
	Иначе
		...
		Возврат Выб.ДолгОстаток;
	КонецЕсли;
КонецФункции

&НаКлиенте
Процедура AMS_ПеренестиДанные(Результат)
	ЗаблокироватьДанныеФормыДляРедактирования();
	...
	AMS_ПеренестиДанныеВДокумент(Результат);
...
&НаСервере
Процедура AMS_ПеренестиДанныеВДокумент(Данные)
	Если Данные.Количество() = 0 Тогда
		Объект.AMS_ДатаСчета = ТекущаяДатаСеанса();
	КонецЕсли;
	...
	Подбор = Док.Товары.ВыгрузитьКолонки("Номенклатура, Упаковка, КоличествоУпаковок, ВидЦены, Цена");
	...
	Товары = Объект.Товары.Выгрузить(, "Номенклатура, ПроцентРучнойСкидки");
	
	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("Подбор", Подбор);
	...
	|	Подбор.Упаковка КАК Упаковка,
	|	Подбор.КоличествоУпаковок КАК КоличествоУпаковок,
	|	ИСТИНА КАК БезВозвратнойТары,
	|	Подбор.ВидЦены КАК ВидЦены,
	...
	|		ПО Подбор.Номенклатура = Ном.Ссылка
	|		ЛЕВОЕ СОЕДИНЕНИЕ Товары КАК Товары
	|		ПО Подбор.Номенклатура = Товары.Номенклатура
	|
	|УПОРЯДОЧИТЬ ПО
	|	Ном.Наименование";
...
&НаКлиенте
Процедура AMS_ОбработатьДокументКонтурEDI(Отказ) //Вызывается из ПриОткрытии()
	ПравилоПродажи = AMS_ВызовСервера.ПолучитьПравилоПродажи(Объект.Дата, Объект.Партнер, Объект.Контрагент);
	
	Если Не ЗначениеЗаполнено(ПравилоПродажи) Тогда 
		Текст = "Для клиента ""%1"" не задано правило продажи";
		Текст = СтрШаблон(Текст, Объект.Партнер);
		ОбщегоНазначенияКлиент.СообщитьПользователю(Текст,,,,Отказ);
		Возврат;
	КонецЕсли;
	
	Объект.AMS_КонтурEDI      = Истина;
	Объект.AMS_ПравилоПродажи = ПравилоПродажи;
	Объект.Грузополучатель    = Неопределено;
	
	AMS_УстановитьПравилаПродажи();
	
	Если СтрНайти(Объект.Основание,"Контур-EDI") > 0 Тогда
		ОснованиеНовое = Объект.Основание;
		Объект.Основание = СтрЗаменить(Объект.Основание, "Контур-EDI", СтрЗаменить(ОснованиеНовое, "0:00:00", "г."));
		Объект.AMS_УсловияОтгрузки = Объект.Основание;
	КонецЕсли;
	
	AMS_ПерезаполнитьТовары();
	
	AMS_ЗаполнитьВесОбъемКолПалет();
КонецПроцедуры

&НаСервере
Процедура AMS_ПерезаполнитьТовары()
	Модифицированность = Истина;
	...
	|	т.ПроцентРучнойСкидки КАК ПроцентРучнойСкидки,
	|	т.ПроцентАвтоматическойСкидки КАК ПроцентАвтоматическойСкидки,
	...
	|	т.СрокПоставки КАК СрокПоставки,
	...
	|	т._ЦенаИзменена КАК _ЦенаИзменена
	|ПОМЕСТИТЬ вт
	|ИЗ
	|	&Товары КАК т
	|;
	...
	|	вт.Количество / Ном.КоэффициентЕдиницыДляОтчетов КАК КоличествоУпаковок,
	...
	|	вт.ПроцентАвтоматическойСкидки КАК ПроцентАвтоматическойСкидки,
	|	вт.КлючСвязи КАК КлючСвязи,
	|	вт.Склад КАК Склад,
	|	вт.СтатусУказанияСерий КАК СтатусУказанияСерий,
	...
	|	вт._ЦенаИзменена КАК _ЦенаИзменена
	|ИЗ
	|	вт КАК вт
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.Номенклатура КАК Ном
...
&НаСервере
Процедура AMS_ЗаполнитьВесОбъемКолПалет()
	Стр = AMS_Сервер.ПолучитьВесОбъемТоваров(Объект.Товары.Выгрузить(,"Номенклатура, Количество"), Объект.AMS_СпособПередачиТоваров);
...
&НаСервере
Процедура AMS_ЗаписатьИсториюРеализации(ТекущийОбъект, ПараметрыЗаписи)	//Вызывается из ПослеЗаписиНаСервере()
	НовоеУтвержденаОтгрузка  = ТекущийОбъект.AMS_УтвержденаОтгрузка;
	СтароеУтвержденаОтгрузка = ТекущийОбъект.ДополнительныеСвойства.AMS_УтвержденаОтгрузка;
	
	Если НовоеУтвержденаОтгрузка = СтароеУтвержденаОтгрузка Тогда
		Если ПараметрыЗаписи.РежимЗаписи = РежимЗаписиДокумента.Запись Тогда
			Событие = Перечисления.AMS_СобытияРеализации.Запись;

		ИначеЕсли ПараметрыЗаписи.РежимЗаписи = РежимЗаписиДокумента.Проведение Тогда
			Событие = Перечисления.AMS_СобытияРеализации.Проведение;
		
		ИначеЕсли ПараметрыЗаписи.РежимЗаписи = РежимЗаписиДокумента.ОтменаПроведения Тогда
			Событие = Перечисления.AMS_СобытияРеализации.ОтменаПроведения;
		КонецЕсли;
	
	Иначе
		Если НовоеУтвержденаОтгрузка Тогда
			Событие = Перечисления.AMS_СобытияРеализации.РазрешитьОтгрузку;
		Иначе
			Событие = Перечисления.AMS_СобытияРеализации.ОтменаРазрешенияОтгрузки;
		КонецЕсли;
	КонецЕсли;
	
	//@skip-check module-self-reference
	РегистрыСведений.AMS_ИсторияРеализации.ЗаписатьСобытиеДокумента(ТекущийОбъект.Ссылка, Событие, ЭтотОбъект.AMS_ВремяНачалаОбработкиДокумента);	
	
	ЭтотОбъект.AMS_ВремяНачалаОбработкиДокумента = ТекущаяДатаСеанса();
КонецПроцедуры

&НаСервере
Процедура AMS_УправлениеЭлементамиФормы()
	AMS_СобытияФорм.УправлениеФормойРеализацияТоваровУслуг(ЭтотОбъект);
КонецПроцедуры

&НаСервере
Процедура AMS_УправлениеРеквизитамиДоставки()
	AMS_СобытияФорм.УправлениеРеквизитамиДоставкиAMS(ЭтотОбъект, "РеализацияТоваровУслуг");
КонецПроцедуры

&НаКлиенте
Процедура AMS_УстановитьПравилаПродажи()
	Если Не ЗначениеЗаполнено(Объект.AMS_ПравилоПродажи) Тогда
		Возврат;
	КонецЕсли;
	
	Правила = AMS_ВызовСервера.ПолучитьДанныеПравилПродажи(Объект.AMS_ПравилоПродажи);
	
	ЗаполнитьЗначенияСвойств(Объект, Правила);
...
&НаКлиенте
Процедура AMS_ВыполнитьКомандуПечати(Док, Макет, Парам = Неопределено)
	Если НЕ ЗначениеЗаполнено(Док) Тогда
		ПоказатьПредупреждение(Неопределено, "Документ ещё не записан." + Символы.ПС + "Печать возможна только после записи документа.");
		Возврат;
	КонецЕсли;
	
	Если Модифицированность Тогда 
		ПоказатьПредупреждение(Неопределено, "Данные документа были изменены." + Символы.ПС + "Печать возможна только после записи изменённых данных.");
		Возврат;
	КонецЕсли;
	
	AMS_Клиент.ВыполнитьКомандуПечати(Объект.Ссылка, Макет, Парам)
КонецПроцедуры

&НаКлиенте
Процедура AMS_ПодборТоваровЗавершение(Результат, ДополнительныеПераметры) Экспорт
	Если Результат = Неопределено Тогда
		Возврат;
	КонецЕсли;
	...
	AMS_ОбработкаПодборНаСервере(Результат);
	
	Если Объект.AMS_ТранспортныеУслуги Тогда 
		Услуга = AMS_ВызовСервера.ПолучитьТранспортныеУслуги(Объект.Партнер, Объект.Товары.Итог("Сумма"), Объект.Товары.Итог("Сумма"));
		
		Стр = Объект.Товары.Добавить();
		
		ЗаполнитьЗначенияСвойств(Стр, Услуга);
		
		Стр.ЗаказКлиента   = Объект.ЗаказКлиента;
		Стр.ОбъектРасчетов = Объект.ОбъектРасчетовУпр;
		Стр.Подразделение  = Объект.Подразделение;
	КонецЕсли;
...
&НаСервере
Процедура AMS_ОбработкаПодборНаСервере(Результат)
	...
	Для каждого Стр Из Результат Цикл
		...
		ТекущаяСтрока.Количество = Стр.КоличествоУпаковок * ОбщегоНазначенияУТВызовСервера.ЗначениеРеквизитаОбъекта(Стр.Номенклатура,"КоэффициентЕдиницыДляОтчетов"); 
...
&НаСервереБезКонтекста
Функция AMS_ПолучитьРезерв(Ссылка, Дата)
	УстановитьПривилегированныйРежим(Истина);
	...
	|	т.КОтгрузкеОстаток / Ном.КоэффициентЕдиницыДляОтчетов КАК Количество
	|ПОМЕСТИТЬ вт
	|ИЗ
	...
	|	вт КАК вт
	|ГДЕ
	|	вт.Дата МЕЖДУ НАЧАЛОПЕРИОДА(&Дата, ДЕНЬ) И КОНЕЦПЕРИОДА(&Дата, ДЕНЬ)";

	Количество = Запрос.Выполнить().Выгрузить()[0].Количество;

	Возврат ?(Количество = Null, 0, Количество);
КонецФункции

&НаСервереБезКонтекста
Функция AMS_ПолучитьСуммуВыполненияДоговора(Договор, Ссылка)
	Запрос = Новый Запрос;
	...
	Иначе
		Выб = Рез.Выбрать(); Выб.Следующий();
		Возврат Выб.СуммаИсполнено;
	КонецЕсли;
КонецФункции

&НаСервере
Процедура AMS_ПроверитьДокументыПередачиТоваров(Отказ)
	Табл1 = Объект.Товары.Выгрузить(, "Номенклатура, Количество");
		
	Если ЗначениеЗаполнено(Объект.AMS_ДокументРеализации) Тогда
		Запрос = Новый Запрос;
		Запрос.УстановитьПараметр("Ссылка", Объект.AMS_ДокументРеализации);
		Запрос.Текст =
		"ВЫБРАТЬ РАЗРЕШЕННЫЕ
		|	т.Номенклатура КАК Номенклатура,
		|	т.Количество КАК Количество
		...
		ТаблицаОтличий = ОбщегоНазначенияУТ.СравнитьТаблицыЗначений(Табл1, Табл2);
		
		Если ТаблицаОтличий.Количество() > 0 Тогда
			Текст = "Содержимое документа ""Реализация товаров и услуг"" в поле ""Передача себе"" не соответствует текущему документу";
			ОбщегоНазначения.СообщитьПользователю(Текст, Объект.Ссылка, "Объект.AMS_ДокументРеализации",, Отказ);
		КонецЕсли;

	Иначе
		Текст = "Не заполнено поле ""Передача себе""";
		ОбщегоНазначения.СообщитьПользователю(Текст, Объект.Ссылка, "Объект.AMS_ДокументРеализации",, Отказ);
	КонецЕсли;
	
	Если ЗначениеЗаполнено(Объект.AMS_ДокументПриобретение) Тогда
		Запрос = Новый Запрос;
		Запрос.УстановитьПараметр("Ссылка", Объект.AMS_ДокументПриобретение);
		Запрос.Текст =
		"ВЫБРАТЬ РАЗРЕШЕННЫЕ
		|	т.Номенклатура КАК Номенклатура,
		|	т.Количество КАК Количество
		...
		Если ТаблицаОтличий.Количество() > 0 Тогда
			Текст = "Содержимое документа ""Приобретение товаров и услуг"" в поле ""Закупка у себя"" не соответствует текущему документу";
			ОбщегоНазначения.СообщитьПользователю(Текст, Объект.Ссылка, "Объект.AMS_ДокументРеализации",, Отказ);
		КонецЕсли;
	
	Иначе
		Текст = "Не заполнено поле ""Закупка у себя""";
		ОбщегоНазначения.СообщитьПользователю(Текст, Объект.Ссылка, "Объект.AMS_ДокументПриобретение",, Отказ);
	КонецЕсли;
КонецПроцедуры

&НаСервере
Процедура AMS_ПроверитьДублиНоменклатуры(Отказ)
	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("Ссылка", Объект.Ссылка);
	Запрос.Текст =
	"ВЫБРАТЬ РАЗРЕШЕННЫЕ
	|	т.Номенклатура КАК Номенклатура,
	|	КОЛИЧЕСТВО(т.Номенклатура) КАК Количество
	|ПОМЕСТИТЬ вт
	|ИЗ
	|	Документ.РеализацияТоваровУслуг.Товары КАК т
	|ГДЕ
	...
	Если Не Запрос.Выполнить().Пустой() Тогда
		Текст = "Содержимое документа имеет повторяющуюся номенклатуру";
		ОбщегоНазначения.СообщитьПользователю(Текст, Объект.Ссылка, "Объект.Товары",, Отказ);
	КонецЕсли;
КонецПроцедуры
