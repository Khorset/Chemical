...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	Если Не ЗначениеЗаполнено(Ссылка) Тогда
		Если СтрНайти(Комментарий, "Контур-EDI") > 0 Тогда
			AMS_КонтурEDI = Истина;
...
Процедура ПриКопировании(ОбъектКопирования)
	...
	AMS_ID                 = 0;
	AMS_ДатаОтгрузки       = Дата(1,1,1);
	AMS_УтвержденаОтгрузка = Ложь;
	AMS_СтатусРеализации   = Перечисления.AMS_СтатусыРеализаций.Заказ;
	AMS_КонтурEDI          = Ложь;
	
	Если ЗначениеЗаполнено(Соглашение) Тогда
		ГрафикОплаты = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Соглашение, "ГрафикОплаты");
		
		Запрос = Новый Запрос;
		Запрос.УстановитьПараметр("ГрафикОплаты", ГрафикОплаты);
		Запрос.Текст =
		"ВЫБРАТЬ
		|	МИНИМУМ(т.Сдвиг) КАК Сдвиг
		|ИЗ
		...
		Если Не Рез.Пустой() Тогда
			...
			Если Сдвиг = 0 Тогда
				AMS_ДатаСчета = ТекущаяДатаСеанса();
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
КонецПроцедуры
...
Процедура ЗаполнитьДокументНаОснованииЗаказаКлиента(Знач ДокументОснование,
	...
	РеализацияТоваровУслугЛокализация.ПроверитьНаличиеСкидокОплатБонуснымиБалламиПоТабачнойПродукции(ЭтотОбъект);
