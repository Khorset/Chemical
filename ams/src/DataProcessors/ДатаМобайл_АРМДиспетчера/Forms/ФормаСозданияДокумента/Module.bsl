...
Процедура СохранитьДокумент(Команда, ПараметрыЗаписи)
	...
	Если Не ВключенаГрупповаяРабота И СписокДокументов.Количество() = 0 Тогда
		ДатаМобайл_ОбщийМодульКлиент.СообщитьПользователю("Укажите исходный документ!");
		Возврат;
	КонецЕсли;	
...
Процедура ДобавитьИсходныеДокументы(Команда)
	...
	Если Не ЗначениеЗаполнено(ШаблонДокумента) Тогда
		ДатаМобайл_ОбщийМодульКлиент.СообщитьПользователю("Выберите шаблон!");
		Возврат;
	КонецЕсли;	
...
Процедура ПередЗаписью(ПараметрыЗаписи)
	...
	Иначе
		СформироватьВопросПоЧестномуЗнаку(ПараметрыЗаписи);	
	КонецЕсли;
...
&НаКлиенте
Процедура СформироватьВопросПоЧестномуЗнаку(ПараметрыЗаписи)

	Если (РеквизитВидДокумента(ШаблонДокумента) = "ПриобретениеТоваровУслуг" 
		Или РеквизитВидДокумента(ШаблонДокумента) = "ПриемкаТоваровИСМП") И ТребуетсяЗапросВЦРПТ(ШаблонДокумента) Тогда
		Оповещение = Новый ОписаниеОповещения("ПоказатьВопросПоЧестномуЗнаку", ЭтаФорма, ПараметрыЗаписи);
				
		ТекстВопроса = "Может быть длительное ожидание получения данных об упаковках и вложенности из ЧЗ (рекомендуем использовать автовыгрузку). Продолжить?"; 
...
&НаКлиенте
Процедура ПоказатьВопросПоЧестномуЗнаку(Результат, ПараметрыЗаписи) Экспорт
...
Функция ТребуетсяЗапросВЦРПТ(Шаблон)
	...
	Если Шаблон.МаркировкаЗапросВЦРПТ И Шаблон.ИспользоватьМаркировку И Шаблон.МаркировкаСоздаватьСправочникКМЕслиНеНайден Тогда
		Возврат Истина;
	Иначе 
		Возврат Ложь;
	КонецЕсли;
