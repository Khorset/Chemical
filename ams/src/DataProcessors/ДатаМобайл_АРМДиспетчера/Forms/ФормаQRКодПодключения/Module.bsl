...
Процедура СформироватьQRкод(Команда)
	...
	Если МассивНеЗаполненных.Количество() <> 0 Тогда
		Для каждого СтрокаНеЗаполненных Из МассивНеЗаполненных Цикл
			ДатаМобайл_ОбщийМодульКлиент.СообщитьПользователю("Параметр " + СтрокаНеЗаполненных + " не заполнен!" + Символы.ПС);	
		КонецЦикла;
		ВывестиQRКод("");
		Возврат;
	КонецЕсли;
...
Процедура ПроверитьДоступностьПубликации(Адрес) 
	...
	Исключение
		...
		КонецЕсли;	
		ДатаМобайл_ОбщийМодульКлиент.СообщитьПользователю(Ошибка);
		Возврат;
	КонецПопытки;
