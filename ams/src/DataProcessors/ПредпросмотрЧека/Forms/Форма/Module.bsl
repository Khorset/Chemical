...
Процедура НастроитьСписокЗначенийВыбораЧасовойЗоныПередачиТовара()
	...
	ЧасовыеЗоныРФПоАдресамПокупателя = ПолучитьЧасовыеЗоныРФПоАдресамПокупателя(ИсточникАдресов);
...
//
&НаСервере
Функция ПолучитьЧасовыеЗоныРФПоАдресамПокупателя(ДокументИсточникАдресов)
...
Процедура ПроверитьКорректностьРозничнойЦеныТабачнойПродукции()
	...
	Для Каждого СтрокаДокументыЕдиногоЧека Из ДокументыЕдиногоЧека Цикл
		...
		Если Не ТипЗнч(СтрокаДокументыЕдиногоЧека.Документ) = Тип("ДокументСсылка.РеализацияТоваровУслуг") Тогда
			Продолжить;
		КонецЕсли;
		
		РеквизитыДокумента = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(
			СтрокаДокументыЕдиногоЧека.Документ, "ШтрихкодыУпаковок,ЦенаВключаетНДС");
		
		РезультатПроверкиСкидок = РозничныеПродажиЛокализация.ПроверитьНаличиеСкидокОплатБонуснымиБаллами(
		...
		ШтрихкодыУпаковок = РеквизитыДокумента.ШтрихкодыУпаковок
				.Выгрузить()
				.ВыгрузитьКолонку("ШтрихкодУпаковки");
		...
		ТоварыДокумента = РозничныеПродажиЛокализация.ТоварыДокументаРеализацииСВидамиПродукцииИС(
			СтрокаДокументыЕдиногоЧека.Документ);
		...
		РезультатПроверки = РезультатПроверкиСкидок
