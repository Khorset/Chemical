...
Функция ДокументыВПроцессеПередачиМеждуОрганизациями(ОбработкаОбъект) Экспорт
	...
	ДатыКонтроля = РегистрыСведений.НастройкиУчетаМаркируемойПродукцииИСМП.ДатыРаздельногоКонтроля();
	ТаблицаДатКонтроля = Новый ТаблицаЗначений;
	ТаблицаДатКонтроля.Колонки.Добавить("ОсобенностьУчета", Новый ОписаниеТипов("ПеречислениеСсылка.ОсобенностиУчетаНоменклатуры"));
	ТаблицаДатКонтроля.Колонки.Добавить("ДатаКонтроля", Новый ОписаниеТипов("Дата"));
	Для Каждого ЭлементМассива Из ДатыКонтроля Цикл
		СтрокаТаблицы = ТаблицаДатКонтроля.Добавить();
		СтрокаТаблицы.ОсобенностьУчета = ИнтеграцияИСУТКлиентСервер.ОсобенностьУчетаПоВидуПродукции(ВидПродукции);
		Если Не ЗначениеЗаполнено(ЭлементМассива.ДатаОптКонтрольОСУ) Тогда
			СтрокаТаблицы.ДатаКонтроля = ЭлементМассива.ДатаОптКонтрольПоштучно;
		ИначеЕсли Не ЗначениеЗаполнено(ЭлементМассива.ДатаОптКонтрольПоштучно) Тогда
			СтрокаТаблицы.ДатаКонтроля = ЭлементМассива.ДатаОптКонтрольОСУ;
		Иначе
			СтрокаТаблицы.ДатаКонтроля = Мин(ЭлементМассива.ДатаОптКонтрольОСУ, ЭлементМассива.ДатаОптКонтрольПоштучно);
		КонецЕсли;
	КонецЦикла;
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	Даты.ОсобенностьУчета КАК ОсобенностьУчета,
	...
	|	ПередачаТоваровМеждуОрганизациямиТовары.Номенклатура.ОсобенностьУчета КАК ОсобенностьУчета
	...
	|	ПО ДокументыПередачиПоОсобеннностямУчета.ОсобенностьУчета = ТаблицаДатКонтроля.ОсобенностьУчета
	|ГДЕ
	|	ТаблицаДатКонтроля.ОсобенностьУчета ЕСТЬ NULL
	...
	|	ПО Товары.Номенклатура.ОсобенностьУчета = ТаблицаДатКонтроля.ОсобенностьУчета
