...
Процедура ЗаполнитьРаспоряженияНаОтгрузку(ПроверятьИзменениеПараметров = Ложь)
	...
	//!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~5
	РаспоряженияНаОтгрузку.Сортировать("Дата, Получатель");
	//!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~6
...
Процедура СоздатьОрдераСервер()
	...
	Если ВыделенныеСтроки.Количество() > 1 Тогда
		ОбщегоНазначения.СообщитьПользователю("Выделите только одну строку для создания ордера!");
		Возврат;
	КонецЕсли;
	
	ПоВсемРаспоряжениям = 1;
	
	Распоряжение = РаспоряженияНаОтгрузку.НайтиПоИдентификатору(ВыделенныеСтроки[0]).Распоряжение;
	
	Если ТипЗнч(Распоряжение) = Тип("ДокументСсылка.ЗаказКлиента") Тогда
		ДатаРаспоряжения = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Распоряжение, "ДатаОтгрузки");
	Иначе
		ДатаРаспоряжения = РаспоряженияНаОтгрузку.НайтиПоИдентификатору(ВыделенныеСтроки[0]).Дата;
	КонецЕсли;
	
	//@skip-check using-isinrole
	Если НачалоДня(ТекущаяДатаСеанса()) > ДатаРаспоряжения 
		И Не (AMS_ВызовСервера.ЭтоKhors() Или РольДоступна("_Бухгалтер")) Тогда
		ОбщегоНазначения.СообщитьПользователю("Дата распоряжения меньше текущей даты!");
		Возврат;
	КонецЕсли;
	//!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~8
