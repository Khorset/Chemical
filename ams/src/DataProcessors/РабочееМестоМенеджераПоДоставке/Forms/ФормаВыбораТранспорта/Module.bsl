...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ДобавляемыеРеквизиты.Добавить(Новый РеквизитФормы("AMS_Водитель", Новый ОписаниеТипов("СправочникСсылка.ФизическиеЛица"), "ТранспортныеСредства", "ТранспортныеСредства", Истина));
...
Процедура ОбновитьСписокТранспорта()
	...
	|	ЛОЖЬ КАК Выбран,
	|	0 КАК ВыбранСчетчик,
	|	ТранспортныеСредства._Водитель КАК AMS_Водитель
	|ИЗ
	|	Справочник.ТранспортныеСредства КАК ТранспортныеСредства
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВТЧастотаИспользованияТранспорта КАК влЧастотаИспользованияТранспорта
	|		ПО влЧастотаИспользованияТранспорта.ТранспортноеСредство = ТранспортныеСредства.Ссылка
	...
	|ГДЕ
	|	НЕ ТранспортныеСредства.ПометкаУдаления
	|	И ТранспортныеСредства._Статус = ЗНАЧЕНИЕ(Перечисление._СтатусыТранспортныхСредств.Работает)";
	//!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~4
