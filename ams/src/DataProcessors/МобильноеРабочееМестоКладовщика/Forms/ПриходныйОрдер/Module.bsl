...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ВидыПродукцииИСМП = Новый Массив(ОбщегоНазначенияИСМПКлиентСерверПовтИсп.УчитываемыеВидыМаркируемойПродукции());
	...
	Если ЗначениеЗаполнено(НомерВходящегоДокумента) И ЗначениеЗаполнено(ДатаВходящегоДокумента) Тогда
		ИнформацияОДокументе = СтрШаблон(НСтр("ru = '№ %1 от %2'"), НомерВходящегоДокумента, Формат(ДатаВходящегоДокумента, НСтр("ru='ДФ=''дд.ММ.гг ЧЧ:мм'''")));
	Иначе
		ИнформацияОДокументе = СтрШаблон(НСтр("ru = '№ %1 от %2'"), ПрефиксацияОбъектовКлиентСервер.НомерНаПечать(Номер), Формат(Дата, НСтр("ru='ДФ=''дд.ММ.гг ЧЧ:мм'''")));
	КонецЕсли;
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если Источник = "ПодключаемоеОборудование" И ВводДоступен() Тогда
		...
		Если ИмяСобытия = "ScanData" И МенеджерОборудованияУТКлиент.ЕстьНеобработанноеСобытие() Тогда
			...
			ПараметрыФормы.Вставить("Серия", ПараметрыКарточкаТовара.Серия);
