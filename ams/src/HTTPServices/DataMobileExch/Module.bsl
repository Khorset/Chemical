...
Функция API(Запрос)
	...
	ВремяВызова = ДатаМобайл_ОбщийМодуль.ПолучитьТекущуюДату();
...
Процедура ЗаписатьВЛог(Запрос, Ответ, Результат, Роутинг, ОписаниеОшибки, КаталогСохраненияЛогов, ВремяВызова)
	...
	ДатаВМиллисекундахУниверсальная = ТекущаяУниверсальнаяДатаВМиллисекундах();
	
	ДатаВМиллисекундах = ДатаВМиллисекундахУниверсальная - Цел(ДатаВМиллисекундахУниверсальная / 1000) * 1000;
	...
	ВремяОтвета = ДатаМобайл_ОбщийМодуль.ПолучитьТекущуюДату();
	ВремяОтветаШтампЗаписи = Число(Формат(ВремяОтвета - Дата(1970, 1, 1, 1, 0, 0), "ЧГ=0"));
	ФорматВремениПодЛог = Формат(ДатаМобайл_ОбщийМодуль.ПолучитьТекущуюДату(), "ДФ='yyyy_MM_dd_HH_mm_ss'") + "_" + ДатаВМиллисекундах + "_";
	...
	Если ЗначениеЗаполнено(ОписаниеОшибки) Тогда
		LogИмяФайла = КаталогСохраненияЛогов + ФорматВремениПодЛог + "error" + "_1C.log";
		...
		Пока ТекущийФайл.Существует() И Сч < 20 Цикл
			...
			LogИмяФайла = КаталогСохраненияЛогов + ФорматВремениПодЛог + Сч + "_" + "error" + "_1C.log";
	...
	Иначе	
		...
		LogИмяФайла = КаталогСохраненияЛогов + ФорматВремениПодЛог + LogИмяМетода + LogРоутинг + ErrorТэг + "_1C.log";
		...
		Пока ТекущийФайл.Существует() И Сч < 20 Цикл
			Сч = Сч + 1;
			LogИмяФайла = КаталогСохраненияЛогов + ФорматВремениПодЛог + Сч + "_" + LogИмяМетода + LogРоутинг + ErrorТэг + "_1C.log";
			ТекущийФайл = Новый Файл(LogИмяФайла);
		КонецЦикла;
...
Функция ОбработатьОбъектArts(Запрос, СтруктураПараметровЗапроса, Роутинг)
	...
	ИначеЕсли МетодЗапроса = "POST" Тогда
		...
		Если ИмяМетода = "/barcodes/generate" Тогда
			...
			unit_coef = СтруктураHttpПараметров.unit_coef;
			...
			Ответ = ДатаМобайл_HTTP_Логика.GenerateNewBC(device_id, username, art_id, unit_id, p1, p2, unit_coef); 
		...
		ИначеЕсли ИмяМетода = "/scan" Тогда 			
			...
			printingPlannedDays	= СтруктураHttpПараметров.printing_planned_days;
			art_id			    = СтруктураHttpПараметров.art_id;
			...
			Если ЗначениеЗаполнено(obj_kiz) Тогда 
				...
				СтруктураДанных.Вставить("printingPlannedDays", printingPlannedDays);
			...
			Иначе
				Если ЗначениеЗаполнено(Barcode) И ЗначениеЗаполнено(doc_id) Тогда
					...
					Если operation_type = 0 Тогда  // Select
						...
						//{Scanport_начало фрагмента
						Params = Новый Структура;
						Params.Вставить("gs1", gs1);
						Params.Вставить("printingPlannedDays", printingPlannedDays);
						Params.Вставить("art_id", art_id);
						//}
						
						Ответ = ДатаМобайл_HTTP_Логика.OnArtScanSelect(device_id, username, barcode, doc_id, cell_barcode, Params);
					...
					ИначеЕсли operation_type = 1 Тогда // Insert
						...
						Ответ = ДатаМобайл_HTTP_Логика.OnArtScanInsert(device_id, username, barcode, doc_id, cell_barcode, Params);
						
					КонецЕсли;
					
				//{Scanport_начало фрагмента	
				ИначеЕсли ЗначениеЗаполнено(Barcode) Или ЗначениеЗаполнено(art_id) Тогда
				//}
				
					Роутинг = "GetArtOnBC";
					
					//{Scanport_начало фрагмента
					Если ЗначениеЗаполнено(printingPlannedDays) Или ЗначениеЗаполнено(art_id) Тогда                                     
						
						Params = Новый Структура;
						Params.Вставить("printingPlannedDays", printingPlannedDays);
						Params.Вставить("art_id", art_id); 
						Params.Вставить("Barcode", Barcode);
						
						Ответ = ДатаМобайл_HTTP_Логика.GetArtOnBC(device_id, username, Params);
...
Функция ОбработатьОбъектClients(Запрос, СтруктураПараметровЗапроса, Роутинг)
	...
	ИначеЕсли ИмяМетода = "/parts" Тогда  	
		...
		Ответ = ДатаМобайл_HTTP_Логика.GetClients(device_id, username, part);
...
Функция ОбработатьОбъектPrint(Запрос, СтруктураПараметровЗапроса, Роутинг)
	...
	Если МетодЗапроса = "GET" Тогда
		...
		Если ИмяМетода = "/data/check" Тогда  
			
			ПолучитьHttpПараметрыПоМетодамAPI(Запрос, СтруктураПараметровЗапроса, СтруктураHttpПараметров);
			Роутинг = "GetPrintDataCheck";
			
			Ответ = ДатаМобайл_HTTP_Логика.GetPrintDataCheck();
			
		ИначеЕсли ИмяМетода = "/data" Тогда  
			
			ПолучитьHttpПараметрыПоМетодамAPI(Запрос, СтруктураПараметровЗапроса, СтруктураHttpПараметров);
			Роутинг = "GetPrintData";
			
			Ответ = ДатаМобайл_HTTP_Логика.GetPrintData();
	...
	ИначеЕсли МетодЗапроса = "POST" Тогда	
		...
		Если ИмяМетода = "" Тогда
			...
			price			   = СтруктураHttpПараметров.price;
			...
			Иначе
				...
				Ответ = ДатаМобайл_HTTP_Логика.SendArtToPrint(device_id, username, art_id, barcode, qty, qty_int, qty_fraction, art_sn, unit_name, is_km, gs1, barcode_full, is_need_print_copy, price);
				
			КонецЕсли;
			
		ИначеЕсли ИмяМетода = "/doc" Тогда  
			...
			username  = СтруктураHttpПараметров.username; 
			doc_id    = СтруктураHttpПараметров.doc_id;
			copy_qty    = СтруктураHttpПараметров.qty;

			Роутинг = "SendDocToPrint";
			
			Ответ = ДатаМобайл_HTTP_Логика.SendDocToPrint(device_id, username, doc_id, copy_qty);
			
		ИначеЕсли ИмяМетода = "/art" Тогда
			...
			art_id 			   = СтруктураHttpПараметров.art_id;
			doc_id 			   = СтруктураHttpПараметров.doc_id;
			sn_data 		   = СтруктураHttpПараметров.sn_data;
			price			   = СтруктураHttpПараметров.price;
			
			// неиспользуемые (для совместимости)
			unit_name    = "";
			barcode_full = "";
			
			Роутинг = "SendArtToPrint";
			
			Ответ = ДатаМобайл_HTTP_Логика.SendArtToPrint(device_id, username, art_id, barcode, qty, qty_int, qty_fraction, art_sn, unit_name, is_km, gs1, barcode_full, is_need_print_copy, price, doc_id, sn_data);
			
		ИначеЕсли ИмяМетода = "/pack" Тогда
...
Функция ОбработатьОбъектFastAccess(Запрос, СтруктураПараметровЗапроса, Роутинг)
	...
	Если МетодЗапроса = "GET" Тогда
		...
		Если ИмяМетода = "" Или ИмяМетода = "/text" Или ИмяМетода = "/url" Или ИмяМетода = "/html" Тогда
			...
			barcode  	= СтруктураHttpПараметров.barcode; 
			doc_id   	= СтруктураHttpПараметров.doc_id;
			gs1_barcode = СтруктураHttpПараметров.gs1_barcode;
			...
			Иначе
				...
				Ответ = ДатаМобайл_HTTP_Логика.FastAccess(device_id, username, barcode, , ИмяМетода, ВерсияПО, gs1_barcode);
	...
	ИначеЕсли МетодЗапроса = "POST" Тогда	
		...
		Если ИмяМетода = "" Тогда // пока только json (Или ИмяМетода = "/text" Или ИмяМетода = "/url" Или ИмяМетода = "/html") 
			
			ПолучитьHttpПараметрыПоМетодамAPI(Запрос, СтруктураПараметровЗапроса, СтруктураHttpПараметров);
			
			device_id  	= СтруктураHttpПараметров.device_id; 
			username 	= СтруктураHttpПараметров.username;			 
			barcodes  	= СтруктураHttpПараметров.rfids;
			
			Роутинг = "FastAccessArray";
			
			Ответ = ДатаМобайл_HTTP_Логика.FastAccessArray(device_id, username, barcodes, , ИмяМетода, ВерсияПО);
	...
	ИначеЕсли ИмяОбъекта = "arts" Тогда		
		...
		Если МетодЗапроса = "GET" Тогда
			...
			Если ИмяМетода = "" Тогда 
				...
				//{Scanport_начало фрагмента
				СтруктураHttpПараметров.Вставить("printing_planned_days", "");
		...
		ИначеЕсли МетодЗапроса = "POST" Тогда
			...
			Если ИмяМетода = "/barcodes/generate" Тогда
				...
				СтруктураHttpПараметров.Вставить("unit_coef", "");
		...
		ИначеЕсли МетодЗапроса = "PUT" Тогда
			...
			ИначеЕсли ИмяМетода = "" Тогда
				...
				СтруктураHttpПараметров.Вставить("rest",           0);
	...
	ИначеЕсли ИмяОбъекта = "fast_access" Тогда
		...
		Если МетодЗапроса = "GET" Тогда
			...
			Если ИмяМетода = "" Или ИмяМетода = "/text" Или ИмяМетода = "/url" Или ИмяМетода = "/html" Или ИмяМетода = "/FastAccessSerialNumber" Тогда
				...
				СтруктураHttpПараметров.Вставить("doc_id",    		"");	
				СтруктураHttpПараметров.Вставить("barcode",   		"");	
				СтруктураHttpПараметров.Вставить("gs1_barcode",   	"");
		...
		ИначеЕсли МетодЗапроса = "POST" Тогда
			...
			Если ИмяМетода = "" Тогда
				...
				СтруктураHttpПараметров.Вставить("device_id", "");
				СтруктураHttpПараметров.Вставить("username",  "");					
				СтруктураHttpПараметров.Вставить("rfids",     Новый Массив);
				//СтруктураHttpПараметров.Вставить("doc_id",    "");
	...
	ИначеЕсли ИмяОбъекта = "print" Тогда
		...
		ИначеЕсли МетодЗапроса = "POST" Тогда	
			...
			ИначеЕсли ИмяМетода = "/doc" Тогда
				...
				СтруктураHttpПараметров.Вставить("username",     		"");
				СтруктураHttpПараметров.Вставить("doc_id",       		"");
				СтруктураHttpПараметров.Вставить("qty",		       		 1);
			...
			ИначеЕсли ИмяМетода = "/art" Тогда
				...
				СтруктураHttpПараметров.Вставить("art_id",       		"");
				СтруктураHttpПараметров.Вставить("doc_id",       		""); 
				СтруктураHttpПараметров.Вставить("sn_data",       		"");
				СтруктураHttpПараметров.Вставить("price",  				"");
...
Процедура ПрочитатьВсеHttpПараметры(Запрос, СтруктураПараметровЗапроса, СтруктураHttpПараметров)
	...
	Попытка
		...
		ИначеЕсли ИмяОбъекта = "fast_access" И МетодЗапроса = "POST" Тогда
			
			Если ТипЗнч(ВходящиеПараметры) = Тип("Соответствие") Тогда
				СтруктураHttpПараметров.rfids = ВходящиеПараметры.Получить("rfids");
			КонецЕсли;
