...
Процедура амсУправлениеЭлементамиФормы()
	AMS_СобытияФорм.УправлениеФормаЭлементаПартнеры(ЭтотОбъект);
КонецПроцедуры

&НаКлиенте
Процедура Подключаемый_амсПриИзмененииТранпортнаяУслугаСпособ(Элемент)
	амсУправлениеЭлементамиФормы();
КонецПроцедуры

&НаКлиенте
Процедура Подключаемый_амсПриИзмененииАртикулКлиента(Элемент)
	Если Объект._АртикулКлиента И Объект._АртикулКлиентаПлюс Тогда
		Объект._АртикулКлиентаПлюс = Ложь;
	КонецЕсли;
КонецПроцедуры

&НаКлиенте
Процедура Подключаемый_амсПриИзмененииАртикулКлиентаПлюс(Элемент)
	Если Объект._АртикулКлиентаПлюс И Объект._АртикулКлиента Тогда
		Объект._АртикулКлиента = Ложь;
	КонецЕсли;
КонецПроцедуры

&НаКлиенте
Процедура Подключаемый_амсПриИзмененииШтрихКодЕдиницы(Элемент)
	Если Объект._ШтрихКодЕдиницы И Объект._ШтрихКодУпаковки Тогда
		Объект._ШтрихКодУпаковки = Ложь;
	КонецЕсли;
КонецПроцедуры

&НаКлиенте
Процедура Подключаемый_амсПриИзмененииШтрихКодУпаковки(Элемент)
	Если Объект._ШтрихКодУпаковки И Объект._ШтрихКодЕдиницы Тогда
		Объект._ШтрихКодЕдиницы = Ложь;
	КонецЕсли;
КонецПроцедуры
