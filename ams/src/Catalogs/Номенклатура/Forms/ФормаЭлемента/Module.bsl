...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	...
	НоменклатураЛокализация.ОбработкаПроверкиЗаполненияНаСервере(ЭтотОбъект, Отказ, ПроверяемыеРеквизиты);
...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	мср.ПередЗаписьюНаСервереНоменклатура(ТекущийОбъект, Отказ);
	//!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~1-6
...
&НаКлиенте
Процедура Подключаемый_AMS_ПриИзменении_ИспользуетФабрикат(Элемент)
	амсУправлениеЭлементамиФормы();
КонецПроцедуры

&НаСервере
Процедура амсНаборУпаковокПриИзменении()
	Табл = AMS_Сервер.ПолучитьУпаковкиПродукции(Объект.НаборУпаковок);
	
	Стр = Табл.Найти("упак","Ед");
	
	Если Не Стр = Неопределено Тогда
		Объект.ЕдиницаДляОтчетов            = Стр.ЕдиницаИзмерения;
		Объект.КоэффициентЕдиницыДляОтчетов = Стр.Количество;
		Объект._Упаковка                    = Стр.Упаковка;
	КонецЕсли;
КонецПроцедуры

&НаСервере
Процедура амсУправлениеЭлементамиФормы()
	AMS_СобытияФорм.УправлениеФормаЭлементаНоменклатура(ЭтотОбъект);
КонецПроцедуры
