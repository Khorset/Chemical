...
// * ПериодКонтроляИзменений - Дата - Период контроля изменений
...
Процедура ВыполнитьОтложенноеРаспределениеВзаиморасчетов(
	...
	|	ВтЗаданияКОбработке.Валюта КАК ВалютаРасчетов
	|ПОМЕСТИТЬ ВтКортежиКОбработке
	|ИЗ
	...
	|		ПО ВтКортежиКОбработке.АналитикаУчетаПоПартнерам = ЗаданияКРаспределениюВзаиморасчетов.АналитикаУчетаПоПартнерам
	|		И ВтКортежиКОбработке.ОбъектРасчетов = ЗаданияКРаспределениюВзаиморасчетов.ОбъектРасчетов
	|		И ВтКортежиКОбработке.ВалютаРасчетов = ЗаданияКРаспределениюВзаиморасчетов.Валюта
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.ОбъектыРасчетов КАК ОбъектыРасчетов
	|		ПО ВтКортежиКОбработке.ОбъектРасчетов = ОбъектыРасчетов.Ссылка
	|СГРУППИРОВАТЬ ПО
	|	ВтКортежиКОбработке.АналитикаУчетаПоПартнерам,
	...
	|		И КортежиКОбработке.ВалютаРасчетов = ЗаданияКРаспределениюВзаиморасчетов.Валюта
	|";
	...
	СтруктураОтбораКортежей = Новый Структура("АналитикаУчетаПоПартнерам, ОбъектРасчетов, ВалютаРасчетов");
	Счетчик = 1;
	Для Каждого ТекущийКортеж Из КортежиКОбработке Цикл
	...
	Для НомерПорции = 1 По Счетчик Цикл
		...
		Исключение
			...
			Для Каждого СтрокаКортежа Из КортежиКТекущемуРаспределению Цикл
				...
				ДанныеОшибки.Вставить("Валюта", СтрокаКортежа.ВалютаРасчетов);
...
Процедура ОбработатьТаблицуИзменений(ТаблицаИзменений, ТаблицаПорцийДанных, ИспользуемыеКортежи = Неопределено)
	...
	Для каждого СтрокаИзменения Из ТаблицаИзменений Цикл
		...
		НоваяСтрока.Валюта = СтрокаИзменения.ВалютаРасчетов;
	КонецЦикла;
			...
			Для Каждого СтрокаДокумента Из СтрокиДокумента Цикл
				Если СтрокаДокумента.НомерПорции <> 0 
					Или(СтрокаДокумента.АналитикаУчетаПоПартнерам = СтрокаИсходныхДанных.АналитикаУчетаПоПартнерам
						...
						И СтрокаДокумента.ВалютаРасчетов = СтрокаИсходныхДанных.ВалютаРасчетов) Тогда
					Продолжить;
				КонецЕсли;
				...
				СтруктураОтбораСвязанногоКортежа.Вставить("ВалютаРасчетов", СтрокаДокумента.ВалютаРасчетов);
