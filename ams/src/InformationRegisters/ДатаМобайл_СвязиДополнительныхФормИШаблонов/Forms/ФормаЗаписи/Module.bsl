...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	Если Не ЗначениеЗаполнено(Запись.ДополнительнаяФорма) Тогда
		ДатаМобайл_ОбщийМодульКлиент.СообщитьПользователю("Не заполнена дополнительная форма. Сохранение невозможно.");
		Отказ = Истина;
	КонецЕсли;
	
	Если Не ЗначениеЗаполнено(Запись.Шаблон) Тогда
		ДатаМобайл_ОбщийМодульКлиент.СообщитьПользователю("Не заполнен шаблон. Сохранение невозможно.");
		Отказ = Истина;
	КонецЕсли;
	
	Если Не ЗначениеЗаполнено(Запись.ВыгружаемыйРеквизит) И Не ЗначениеЗаполнено(Запись.ЗаполняемыйРеквизит)  Тогда
		ДатаМобайл_ОбщийМодульКлиент.СообщитьПользователю("Не заполнен реквизит документа. Сохранение невозможно.");
		Отказ = Истина;
	КонецЕсли; 
	
	Если ШаблонСПодчиненнымДокументом() И Запись.РежимВвода = 0 И Не ЗначениеЗаполнено(Запись.ВыгружаемыйРеквизит) Тогда
		ДатаМобайл_ОбщийМодульКлиент.СообщитьПользователю("Не заполнен выгружаемый реквизит документа. Сохранение невозможно.");
		Отказ = Истина;
	КонецЕсли;
	
	Если ЕстьДополнительнаяФормаВЭтомШаблоне() Тогда
		ДатаМобайл_ОбщийМодульКлиент.СообщитьПользователю("Такая дополнительная форма уже добавлена в текущем шаблоне """ + Запись.Шаблон + """");
		Отказ = Истина;
	Иначе
...
Процедура ДоступностьНеобходимостьЗаполнения()
	...
	РежимВводаПоУмолчанию = (Запись.РежимВвода = 0);
...
Процедура ПроверитьПодборРазмещениеПоШаблону(ИзменилсяПодборРазмещение)
	...
	Если Запись.ПодборРазмещение = 1 И Не ИспользоватьПриемку Тогда
		...
		ДатаМобайл_ОбщийМодуль.СообщитьПользователю("В шаблоне не включен параметр ""Размещение"".");
	КонецЕсли; 
	
	Если Запись.ПодборРазмещение = 0 И Не ИспользоватьПодбор Тогда
		Запись.ПодборРазмещение = 1;
		ИзменилсяПодборРазмещение = Истина;
		ДатаМобайл_ОбщийМодуль.СообщитьПользователю("В шаблоне не включен параметр ""Подбор"".");
	КонецЕсли;
...
Процедура ДоступностьВыгружаемыйРеквизит()
	...
	НастройкаДоступна = (Не Запись.Шаблон.ВидДокумента = "" И Не Запись.Шаблон.ВидДокументаНового = "");
	
	Если ЗначениеЗаполнено(Запись.Шаблон) И ЗначениеЗаполнено(Запись.ДополнительнаяФорма) И НастройкаДоступна Тогда
		Элементы.ВыгружаемыйРеквизит.Видимость = НастройкаДоступна;
		
		Если Не НастройкаДоступна И ЗначениеЗаполнено(Запись.ВыгружаемыйРеквизит) Тогда
...
&НаСервере
Функция ШаблонСПодчиненнымДокументом()
	
	Возврат Не Запись.Шаблон.ВидДокументаНового = "";
	
КонецФункции

&НаКлиенте
Процедура ДоступностьПоискаВТЧ()
	
	НастройкаДоступна = (Не Запись.ИзменятьВИсходномДокументе);
	Элементы.ИспользоватьВПоискеСтрок.Доступность = НастройкаДоступна;
	
	Если Не НастройкаДоступна И Запись.ИспользоватьВПоискеСтрок Тогда
		Запись.ИспользоватьВПоискеСтрок = Ложь;
		Модифицированность = Истина;
	КонецЕсли;
...
&НаСервере
Процедура ДоступностьРежимаЗаписи()
	
	НастройкаДоступна = (Не Запись.Шаблон.ВидДокумента = "" И Не Запись.Шаблон.ВидДокументаНового = "");
	Элементы.ИзменятьВИсходномДокументе.Доступность = НастройкаДоступна;
	
	Если Не НастройкаДоступна И Запись.ИзменятьВИсходномДокументе Тогда 
		Запись.ИзменятьВИсходномДокументе = Ложь;
		Модифицированность = Истина;
	КонецЕсли;
...
&НаКлиенте
Процедура ИзменятьВИсходномДокументеПриИзменении(Элемент)
